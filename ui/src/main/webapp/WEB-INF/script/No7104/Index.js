                    ...
                    },
                    {
                        ...
                        handler: function () {
                            if (Ext.ComponentManager.get(
                                ...
                                gridStore.load({
                                    params: {
                                        A_EQU_ID: Ext.ComponentManager.get('xzsb_Id').getValue()
                                    }
                                });
                            }
                        }
                    ...
                    },
                    {
                        ...
                        handler: function () {
                            if (Ext.ComponentManager.get(
                                ...
                                window.open(AppUrl + "/page/No710401/Index.html?EQUCODE=" + Ext.ComponentManager.get('xzsb_Id').getValue(),
                                    "",
    ...
    if (Ext.ComponentManager.get('updateid').getValue() == "") {// 添加
        Ext.Ajax.request({
            ...
            params: {
                ...
                A_EQUID: Ext.ComponentManager.get('xzsb_Id').getValue(),// 设备编码
...
function getEquipReturnValue(ret) {
    ...
    Ext.getCmp('xzsb_Id').setValue(str[0]);
}
...
/*function OnButtonExportClicked() {
    ...
    var parType = ['s'];
    var parVal = [excelProtect(Ext.ComponentManager.get('xzsb_Id').getValue())];
    var proName = 'PRO_RUN_SITE_ALL';
    var ExcelName = '设备备件备件安装位置表';

    var cursorName = 'RET';

    location.href = "ModelExcelTool?" + "tableName=" + tableName.join(",") + "&tableKey=" + tableKey.join(",") + "&parName=" + parName.join(",") + "&parType=" + parType.join(",") + "&parVal=" + parVal.join(",") + "&proName=" + proName + "&ExcelName=" + ExcelName;
