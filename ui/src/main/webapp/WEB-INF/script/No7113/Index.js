...
var sbxzStore = Ext.create('Ext.data.Store', {
    autoLoad : false,
    storeId : 'sbxzStore',
    fields : [ 'EQU_DESC', 'EQU_ID' ],// 'V_V_PLANTCODE', 'V_V_DEPTCODE'
    proxy : {
        type : 'ajax',
        async : false,
...
var siteStore = Ext.create('Ext.data.Store', {
    ...
    proxy : {
        type : 'ajax',
        async : false,
        url : AppUrl + 'zy/PRO_RUN_SITE_BJ_ALL',
        actionMethods : {
            read : 'POST'
        },
        reader : {
...
var equBarGrid = Ext.create('Ext.grid.Panel', {
    ...
    store : {
        ...
        proxy : {
            type : 'ajax',
            async : false,
            url : AppUrl + 'zy/PG_RUN7113_GETORDERMATBARCODE',
            actionMethods : {
                read : 'POST'
            },
            reader : {
...
var grid = Ext.create('Ext.grid.Panel',{
    ...
    store : {
        ...
        proxy : {
            type : 'ajax',
            async : false,
            url : AppUrl + 'zy/PRO_RUN7113_ORDERMATLIST',
            actionMethods : {
                read : 'POST'
            },
            reader : {
                ...
                handler : function() {
                    if (Ext.getCmp('jcnr').getValue() == '') {
                        ...
                        if (Ext.getCmp('upload').getValue() != null|| Ext.getCmp('upload').getValue() != "") {
                            ...
                            //alert('111111');
                            form.submit({url : AppUrl+ 'zy/uploadFile',
                                async : false,
                                waitMsg : '上传中...',
                                params : {
...
function siteChange() {
    ...
    if (data.length == 0) {
        ...
        for ( var i = 0; i < data.length; i++) {
            ...
            Ext.Ajax.request({
                url : AppUrl + 'zy/PRO_RUN7113_CHANGEORDERMAT',
                method : 'POST',
                params : {
                    A_ID : golid,
                    ...
                    //CHANGEDATE : Ext.Date.format(data.D_DATE_EDITTIME,"YYYY-mm-dd HH:ii:ss"),//checktime : Ext.Date.format(Ext.getCmp('changeDate').getValue(),'Y-m-d'),
                    CHANGEDATE : Ext.Date.format(Ext.getCmp('changeDate').getValue(),'Y-m-d'),
        ...
        .request({
            ...
            params : {
                parVal : [
                    ...
                    data.OP_USERID,
                    data.OP_USERNAME,
                    Ext.Date.format(data.D_DATE_EDITTIME,"YYYY-mm-dd HH:ii:ss") ]
            },
            success : function(resp) {
                resp = Ext.decode(resp.responseText);
                if (resp == 'success') {
...
function OnButtonExportClicked() {
    ...
    var V_DEPT_CODE =  Ext.getCmp("zyq").getValue();
    var V_EQUIP_CODE = Ext.getCmp("selEqu").getValue();
    var V_MATERIALCODE = Ext.getCmp("mCode").getValue();
    var V_MATERIALNAME = Ext.getCmp("mName").getValue();

    document.location.href = AppUrl + 'zy/PRO_RUN7113_ORDERMATLIST_excel?V_DEPT_CODE='+ V_DEPT_CODE +
        '&V_EQUIP_CODE='+ V_EQUIP_CODE +
        '&V_MATERIALCODE=' + V_MATERIALCODE +
