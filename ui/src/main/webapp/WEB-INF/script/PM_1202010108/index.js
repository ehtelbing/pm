...
var gridStore = Ext.create('Ext.data.Store', {
	...
	proxy : {
		type : 'ajax',
		async : false,
		url : AppUrl + 'PM_12/PRO_RUN7125_EQUVGLIST',
		actionMethods : {
			read : 'POST'
		},
		reader : {
...
function create() {
	Ext.Ajax.request({
		url : AppUrl + 'PM_12/PRO_RUN7125_ADDEQUVG',
		method : 'POST',
		params : {
			V_EQU_ID:Ext.getCmp('equId').getValue(),
			V_VG_IDL:Ext.getCmp('vgId').getValue()
		},
		success : function(resp) {
			...
			if (resp.OUT_RESULT == 'success') {
				...
				Ext.Msg.alert('操作信息', '设备编码或VG图ID不存在');
			} else {
...
function deleteRecord(equId, vgId) {
	Ext.Ajax.request({
		url : AppUrl + 'PM_12/PRO_RUN7125_DELEQUVG',
		async : false,
		method : 'POST',
		params : {
			V_EQU_ID:equId,
