Ext.onReady(function() {
			...
			var gridStore = Ext.create('Ext.data.Store', {
				...
				autoLoad : false,
				pageSize :100,
				fields : ['SUPPLY_CODE', 'SUPPLY_NAME', 'SUPPLY_DESC', 'SUPPLY_RENAGE',
						'SUPPLY_MANAGER', 'LINK_PERSON','LINK_TYPE', 'LINK_PHONECODE', 'SUPPLY_STATUS' ],
				proxy : {
					type : 'ajax',
					async : false,
					url : AppUrl + 'PM_12/PRO_RUN7124_SUPPLYLIST',
					actionMethods : {
						read : 'POST'
					},
					reader : {
			...
			Ext.getCmp('show').on('click',function(a,b,c,d,e){
				var SUPPLY_CODE = gridStore.getAt(c).get("SUPPLY_CODE");
				var SUPPLY_NAME = gridStore.getAt(c).get("SUPPLY_NAME");
				window.open(AppUrl+"page/PM_120201010701/index.html?SUPPLY_CODE="+SUPPLY_CODE+"&SUPPLY_NAME="+SUPPLY_NAME,'', "dialogWidth:800px;dialogHeight:420px");
			});
...
function btnbc() {
	if (Ext.ComponentManager.get('updateid').getValue() == "") {// 添加
		...
		Ext.Ajax.request({
			url : AppUrl + 'PM_12/PRO_RUN7124_ADDSUPPLY',
			method : 'POST',
			params : {
			...
			success : function(response) {
				...
				if(resp.OUT_RESULT=="success"){
				   ...
				   Ext.Msg.alert('操作信息','添加失败 - 已存在相应 - 供应商编码');
				}
			}

		});
		}else{
		     Ext.Msg.alert('操作信息','《供应商编码》与《供应商名称》 - 不能为空');
		}
	} else {// 修改
		Ext.Ajax.request({
			url : AppUrl + 'PM_12/PRO_RUN7124_UPDATESUPPLY',
			method : 'POST',
			params : {
				'V_SUPPLY_CODE':Ext.ComponentManager.get('V_SUPPLY_CODE').getValue(),// 供应商编码
				'V_SUPPLY_NAME':Ext.ComponentManager.get('V_SUPPLY_NAME').getValue(),// 供应商名称
