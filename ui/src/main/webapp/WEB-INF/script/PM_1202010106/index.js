...
var gridStore = Ext.create('Ext.data.Store', {
	...
	proxy : {
		type : 'ajax',
		async : false,
		url : AppUrl + 'PM_12/PRO_RUN7123_SELECTSTLIST',
		actionMethods : {
			read : 'POST'
		},
		reader : {
			...
			listeners : {
				click : function() {
					...
					if (length != 1) {
						...
						Ext.getCmp('tagId').setValue(selectedRecord.TAG_ID);
						Ext.getCmp('tagDesc').setValue(selectedRecord.TAG_DESC);
						Ext.getCmp('tagUnit').setValue(selectedRecord.TAG_UNIT);
						Ext.getCmp('status').setValue(selectedRecord.STATUS);
					}
					Ext.ComponentManager.get('operateType').setValue('modify');
...
function onPageLoaded() {
	...
	Ext.data.StoreManager.lookup('selSiteStore').on("load", function() {
		Ext.getCmp("selSite").select(Ext.data.StoreManager.lookup('selSiteStore').getAt(0));
	});
...
function saveClick() {
	...
	if (desc == "" || desc == null || unit == "" || unit == null) {
		Ext.Msg.alert('操作信息', '请填写指标名称和指标计量单位');
	} else {
...
function create() {
	...
	if (!checkDescSize(desc)) {
		Ext.Msg.alert('操作信息', '指标名称不能超过50个字符');
	} else if (!checkUnitSize(unit)) {
		Ext.Msg.alert('操作信息', '指标计量单位不能超过8个字符');
	} else {
		Ext.Ajax.request({
			url : AppUrl + 'PM_12/PRO_RUN7123_ADDST',
			method : 'POST',
			params : {
				'V_SITE_ID': Ext.getCmp('selSite').getValue(),
...
function modify() {
	...
	if (!checkDescSize(desc)) {
		...
		Ext.Ajax.request({
			url : AppUrl + 'PM_12/PRO_RUN7123_UPDATEST',
			method : 'POST',
			params : {
...
function changeStatus(code, status) {
	if (status == '1') {
		Ext.Ajax.request({
			url : AppUrl + 'PM_12/PRO_RUN7123_STOPST',
			async : false,
			method : 'POST',
			params : {
				V_EQU_ID:code
		...
		Ext.Ajax.request({
			url : AppUrl + 'PM_12/PRO_RUN7123_STARTST',
			async : false,
			method : 'POST',
			params : {
				V_EQU_ID:code
...
function getNowDevice(){
	var ret = window.open(AppUrl + 'page/PM_090101/index.html?DEPTCODE=' + Ext.ComponentManager.get('selSection').getValue() + '', '', 'dialogHeight:500px;dialogWidth:800px');
}
