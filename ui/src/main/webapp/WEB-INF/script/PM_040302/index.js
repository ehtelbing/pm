...
function Add(){
    ...
    Ext.getCmp('winprocode').setReadOnly(false);
    Ext.getCmp('winproname').setReadOnly(false);
    Ext.getCmp('winwbscode').setReadOnly(false);
}
function Edit(){
    flag='edit';
    ...
    if (seldata.length != 1) {
        ...
        Ext.getCmp('winprocode').setReadOnly(true);
        Ext.getCmp('winproname').setReadOnly(true);
        Ext.getCmp('winwbscode').setReadOnly(true);
    }
...
function Delete(){
    ...
    Ext.Ajax.request({
        ...
        method: 'POST',
        params: {
            V_V_PROJECTCODE:seldata[0].data.V_PROJECT_CODE,
            V_V_WBSCODE:seldata[0].data.V_WBS_CODE

        },
        success: function (resp) {
            var resp = Ext.decode(resp.responseText);
            if(resp!=''&&resp.list[0].V_INFO == 'success'){
                alert("删除成功");
                queryGrid();
            }else{
                alert("删除失败");
