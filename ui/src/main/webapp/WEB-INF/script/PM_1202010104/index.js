...
var gridStore = Ext.create('Ext.data.Store', {
	...
	proxy : {
		type : 'ajax',
		async : false,
		url : AppUrl + 'PM_12/PRO_RUN7121_SELECTEQULIST',
		actionMethods : {
			read : 'POST'
		},
		reader : {
			...
			listeners : {
				click : function() {
					...
					if (length != 1) {
						...
						Ext.getCmp('equId').setValue(selectedRecord.EQU_ID);
						Ext.getCmp('equName').setValue(selectedRecord.EQU_DESC);
						Ext.getCmp('userId').setValue(selectedRecord.USERID);
						Ext.getCmp('userName').setValue(selectedRecord.USERNAME);
						Ext.getCmp('sysCode').setValue(selectedRecord.PP_CODE);
						Ext.getCmp('status').setValue(selectedRecord.STATUS);
					}
					Ext.ComponentManager.get('equId').setReadOnly(true);
...
function saveClick() {
	...
	if (equId == "" || equId == null || equName == "" || equName == null) {
		Ext.Msg.alert('操作信息','请填写设备ID和设备名称');
	} else {
...
function create() {
	Ext.Ajax.request({
		url : AppUrl + 'PM_12/PRO_RUN7121_GETEQULIST',
		method : 'POST',
		params : {
		...
		success : function(resp) {
			...
			if (resp.OUT_RESULT!= "1") {
				Ext.Ajax.request({
					url : AppUrl + 'PM_12/PRO_RUN7121_ADDEQU',
					method : 'POST',
					params : {
						'V_EQU_ID':Ext.getCmp('equId').getValue(),
				...
				Ext.Msg.alert('操作信息', '设备编号已存在请重新填写');
			}
		}
	});
...
function modify() {
	Ext.Ajax.request({
		url : AppUrl + 'PM_12/PRO_RUN7121_UPDATEEQU',
		method : 'POST',
		params : {
			'V_EQU_ID':Ext.getCmp('equId').getValue(),
			'V_EQU_DESC':Ext.getCmp('equName').getValue(),
...
function changeStatus(code, status) {
    if(status == '1'){
		Ext.Ajax.request({
			url: AppUrl + 'PM_12/PRO_RUN7121_STOP',
			async: false,
			method: 'POST',
			params: {
				V_EQU_ID:code
		...
		Ext.Ajax.request({
			url: AppUrl + 'PM_12/PRO_RUN7121_STARTEQU',
			async : false,
			method : 'POST',
			params : {
				V_EQU_ID:code
