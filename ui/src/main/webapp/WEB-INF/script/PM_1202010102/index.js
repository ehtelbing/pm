					...
					.create(
							'Ext.panel.Panel',
							{
								...
								items : [
										...
										handler : function() {
											if (Ext.ComponentManager.get('xzsb').getValue() == "") {
												Ext.Msg.alert('操作信息','请点击选择设备进行添加');
											} else {
												windows.show();
										...
										handler : function() {
											...
											if (length == 0) {
												...
												Ext.Msg.confirm("警告", "确定要删除吗？", function(button) {
													...
													for (i = 0; i < Ext.getCmp('grid').getSelectionModel().getSelection().length; i++) {
														Ext.Ajax.request({
															url : AppUrl + 'PM_12/PRO_RUN_SITE_DELETE',
															method : 'POST',
															params : {
										...
										},*/
										{
											...
											handler : function() {
												if (Ext.ComponentManager.get('equcode').getValue() == "") {
													Ext.Msg.alert('操作信息','请选择设备');
												} else {
													window.open(AppUrl+ "page/PM_120201010201/index.html?EQUCODE="+ Ext.ComponentManager.get('equcode').getValue(), "", "dialogHeight:500px;dialogWidth:800px");
												}
											}
										} ]
							});
...
function btnbc() {
	if (Ext.ComponentManager.get('updateid').getValue() == "") {// 添加
		Ext.Ajax.request({
			url : AppUrl + 'PM_12/PRO_RUN_SITE_ADD',
			method : 'POST',
			params : {
				...
				'A_BJ_AMOUNT':Ext.ComponentManager.get('bjamount').getValue()//更换数量
			},
			success : function(response) {
				var resp = Ext.JSON.decode(response.responseText);
				if(resp.RET=='Success'){
					Ext.Msg.alert('操作信息', '添加位置成功');
				}
			}
		});
	} else {// 修改
		Ext.Ajax.request({
			url : AppUrl + 'PM_12/PRO_RUN_SITE_UPDATE',
			method : 'POST',
			params : {
				...
				'A_MEND_USERNAMEID':Ext.ComponentManager.get('fzrId').getValue(), // 维修负责人ID
				'A_BJ_ID':Ext.ComponentManager.get('ksybjbh').getValue(),// 备件
				'A_BJ_AMOUNT':Ext.ComponentManager.get('bjamount').getValue()  //数量
			},
			success : function(response) {
				var resp = Ext.JSON.decode(response.responseText);
				if(resp.RET=='Success'){
					Ext.Msg.alert('操作信息', '修改位置成功');
				}
			}
		});
	}
...
function getNowDevice() {
	window.open(AppUrl + "page/PM_090101/index.html?DEPTCODE="+Ext.getCmp('zyq').getValue(),'', "dialogHeight:400px;dialogWidth:800px");
}
