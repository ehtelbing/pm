...
function _preInsertCrinterion() {
    ...
    var V_V_CKTYPE = 'ZYDQ';
    var Crinterion = window.showModalDialog(AppUrl + 'page/PM_01010301/index.html?V_V_CKTYPE=' + V_V_CKTYPE + '&V_EQUTYPECODE=' + V_EQUTYPECODE + '&V_EQUTYPENAME=' + V_EQUTYPENAME + '&V_V_DEPTCODE=' + Ext.getCmp('V_V_DEPTCODE').getSubmitValue() + '&random=' + Math.random(), window, 'resizable=yes; dialogWidth=560px; dialogHeight=550px');
    if (Crinterion != null) {
        _seltctCriterion(V_EQUTYPECODE);
...
function _preUpdateCrinterion() {
    ...
    var Crinterion = window.showModalDialog(AppUrl + 'page/PM_01010302/index.html?V_V_CRITERION_CODE=' + records[0].get('V_CRITERION_CODE') + '&random=' + Math.random(), window, 'resizable=yes; dialogWidth=560px; dialogHeight=550px');
    if (Crinterion != null) {
        _seltctCriterion(V_EQUTYPECODE);
        top.banner.Ext.example.msg('操作信息', '操作成功');
    }
