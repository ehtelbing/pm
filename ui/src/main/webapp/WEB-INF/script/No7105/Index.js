	...
	.onReady(function() {
		var gridStore = Ext.create('Ext.data.Store', {
			...
			proxy : {
				type : 'ajax',
				async : false,
				url: AppUrl + 'cjy/PRO_RUN_CYCLE_ALL',
				actionMethods : {
					read : 'POST'
				},
				reader : {
							...
							},
							{
								...
								handler : function() {
									if (Ext.ComponentManager
										...
										Ext.Ajax.request({
											url: AppUrl + 'cjy/PRO_RUN_CYCLE_ADD',
											async: false,
											method: 'POST',
											params: {
							...
							},
							{
								...
								handler : function() {
									if (Ext.ComponentManager
										...
										Ext.Ajax.request({
											url: AppUrl + 'cjy/PRO_RUN_CYCLE_UPDATE',
											async: false,
											method: 'POST',
											params: {
										...
										Ext.Msg
											...
											function(
												...
												for (i = 0; i < Ext
													...
													Ext.Ajax.request({
														url: AppUrl + 'cjy/PRO_RUN_CYCLE_DELETE',
														async: false,
														method: 'POST',
														params: {
															A_CYCLE_ID:selectModel
																.getSelection()[i].data.CYCLE_ID
														},
														success: function (ret) {
															var resp = Ext.JSON.decode(ret.responseText);
															if(resp.RET=="Fail"){
		...
		Ext.ComponentManager.get('xz').on(
			"click",
			function(view, rowIndex, colIndex, item, e) {
				Ext.ComponentManager.get('zqms').setValue(
					Ext.getStore("gridStore").getAt(colIndex).get(
						'CYCLE_DESC')),// 周期描述
					Ext.ComponentManager.get('jsdw').setValue(
						Ext.getStore("gridStore").getAt(colIndex).get(
							'CYCLE_UNIT'))// 计算单位
