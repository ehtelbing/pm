...
var equBarGrid = Ext.create('Ext.grid.Panel', {
	...
	store : {
		...
		proxy : {
			type : 'ajax',
			async : false,
			url : AppUrl + 'PM_12/pg_run7113_getordermatbarcode',
			actionMethods : {
				read : 'POST'
			},
			reader : {
...
var grid = Ext.create('Ext.grid.Panel',{
					...
					store : {
						...
						proxy : {
							type : 'ajax',
							async : false,
							url : AppUrl + 'PM_12/PRO_RUN7113_ORDERMATLIST',
							actionMethods : {
								read : 'POST'
							},
							reader : {
										...
										},
										{
											...
											handler : function() {
												if (Ext.getCmp('jcnr').getValue() == '') {
													Ext.Msg.alert('操作信息','请填写检查内容!');
												} else {
													if (Ext.getCmp('upload').getValue() != null|| Ext.getCmp('upload').getValue() != "") {
														var form = this.up('form').getForm();
														form.submit({url : AppUrl+ 'PM_12/uploadImageFile',
																	async : false,
																	waitMsg : '上传中...',
																	params : {
...
function siteChange() {
	...
	if (data.length == 0) {
		...
		for ( var i = 0; i < data.length; i++) {
			...
			Ext.Ajax.request({
				url : AppUrl + 'PM_12/PRO_RUN7113_CHANGEORDERMAT',
				method : 'POST',
				params : {
					'A_ID':golid,
					...
					'CHANGEDATE':Ext.Date.format(data.D_DATE_EDITTIME,"YYYY-mm-dd HH:ii:ss"),
...
function OnClickDeleteLink() {
	...
	Ext.Ajax.request({
				url : AppUrl + 'PM_12/PRO_RUN7113_CHANGECANCEL',
				method : 'POST',
				params : {
					V_I_ID:data.I_ID,
					...
					V_CHANGETIME:Ext.Date.format(data.D_DATE_EDITTIME,"YYYY-mm-dd HH:ii:ss")
				},
				success : function(resp) {
					resp = Ext.decode(resp.responseText);
					if (resp.RET== 'success') {
...
var filter = function(record, id) {
	...
	if (sbname == '' || sbname == null) {
		...
		if (record.get("EQU_DESC") && record.get("EQU_DESC").indexOf(sbname) >= 0)
			return true;
		else
			return false;
	}
