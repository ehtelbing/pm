...
Ext.onReady(function () {
	...
	var gridStore = Ext.create('Ext.data.Store', {
		...
		proxy:  Ext.create("Ext.ux.data.proxy.Ajax", {
			...
			url: AppUrl + 'hp/PRO_PM_03_PLAN_QUARTER_GET',
			actionMethods: {
				read: 'POST'
			},
			reader: {
						...
						items: [
							{
								xtype: 'button',
								text: '计划选择',
								style: 'margin: 10px 0px 0px 312px',
...
function _zyq_sbmc() {
	...
	if(UPDATE == 'update' && UPDATELOAD)
	{
		...
		/*if(gridStore.getAt(0).get('V_QUARTER') == 1)
		{
			Ext.getCmp('jd').setValue("春季");
		}
		if(gridStore.getAt(0).get('V_QUARTER') == 2)
		{
			Ext.getCmp('jd').setValue("夏季");
		}
		if(gridStore.getAt(0).get('V_QUARTER') == 3)
		{
			Ext.getCmp('jd').setValue("秋季");
		}
		if(gridStore.getAt(0).get('V_QUARTER') == 4)
		{
			Ext.getCmp('jd').setValue("冬季");
		}*/
		Ext.getCmp('jd').setValue(gridStore.getAt(0).get('V_QUARTER').toString());
