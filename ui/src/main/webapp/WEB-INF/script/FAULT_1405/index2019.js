...
if (location.href.split('?')[1] != undefined) {
    ...
    (parameters.v_workorder_guid == undefined) ? v_workorder_guid = '%' : v_workorder_guid = parameters.v_workorder_guid;
    ...
    var faultItemPanel = Ext.create('Ext.grid.Panel', {
        ...
        columns: [{
            ...
            renderer: function (value, metaData, record, rowIdx, colIdx, store, view) {
                return '<a href="#" onclick="_glFault(\'' +record.data.V_GUID+ '\')">关联事故</a>';
            }
        },{
            text: '状态',
    ...
    Ext.ComponentManager.get('foundview')
        .on(
            ...
            function(view, rowIndex, colIndex, item, e) {
                ...
                window.open(AppUrl + "page/FAULT_1405/workorderlist.html?faultguid=" + data.V_GUID,
...
function _addOpen() {
    Ext.Ajax.request({//是否已关联事故
        url: AppUrl + 'cxy/PM_WORKORDER_FAULT_GL_SEL',
        type: 'post',
        async: false,
        params: {
            V_WORKORDER_GUID: v_workorder_guid
        ...
        success: function (response) {
            ...
            if (resp.num==0) {
                ...
                window.open(AppUrl + 'page/FAULT_1405/workordertofault.html?V_V_ORGCODE='
                    + Ext.getCmp('V_V_ORGCODE').getValue()+'&V_V_DEPTCODE='+Ext.getCmp('V_V_DEPTCODE').getValue()+'&v_workorder_guid=' + v_workorder_guid
...
function _deleteFaultGL() {
    Ext.MessageBox.show({
        ...
        fn: function (btn) {
            if (btn == 'yes') {
                Ext.Ajax.request({
                    url: AppUrl + 'cxy/PM_WORKORDER_FAULT_GL_DEL',
                    type: 'ajax',
                    method: 'POST',
                    params: {
                        'V_WORKORDER_GUID': v_workorder_guid,
...
function _glFault(V_FAULT_GUID) {
    Ext.Ajax.request({//是否已关联事故
        url: AppUrl + 'cxy/PM_WORKORDER_FAULT_GL_SEL',
        type: 'post',
        async: false,
        ...
        success: function (response) {
            var resp=Ext.decode(response.responseText);
            if (resp.num==0) {
                Ext.Ajax.request({//关联事故
                    url: AppUrl + 'cxy/PM_WORKORDER_FAULT_SET',
                    type: 'ajax',
                    method: 'POST',
                    params: {
                        'V_V_FAULT_GUID': V_FAULT_GUID,
                        'V_V_WORKORDER_ORDERID': v_workorder_guid,
