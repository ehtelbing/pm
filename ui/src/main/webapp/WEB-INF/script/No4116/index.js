...
Ext.onReady(function () {
    ...
    var panel = Ext.create('Ext.panel.Panel', {
        ...
        items: [{
            ...
            {xtype: 'button', text: '批量打印', width: 85, listeners: {click: OnButtonCreateBillClicked}},
            {
                ...
                icon: imgpath + '/excel.gif',
                width: 120,
                listeners: {click: unMatExcelButton}
            },
    ...
    Ext.data.StoreManager.lookup('sgdzt').on("load", function () {
        Ext.getCmp('gdzt').store.insert(0, {
            'V_STATECODE': '%',
            'V_STATENAME': '全部'
        });
...
function OnClickExcelButton() {
    ...
    var repaircode = Ext.getCmp("jxzyq").getValue() == '%' ? '0' : Ext.getCmp("jxzyq").getValue();
    var statecode = Ext.getCmp("gdzt").getValue() == '%' ? '0' : Ext.getCmp("gdzt").getValue();
    var typecode = Ext.getCmp("sblx").getValue() == '%' ? '0' : Ext.getCmp("sblx").getValue();
    var equcode = Ext.getCmp("sbmc").getValue() == '%' ? '0' : Ext.getCmp("sbmc").getValue();
    var djper = Ext.getCmp("djy").getValue() == '%' ? '0' : Ext.getCmp("djy").getValue();
    document.location.href = AppUrl + 'excel/GDCX_EXCEL2?V_D_ENTER_DATE_B=' + Ext.Date.format(Ext.getCmp("begintime").getValue(), 'Y-m-d') +
        ...
        '&V_V_DEPTCODEREPARIR=' + repaircode +
        '&V_V_STATECODE=' + statecode +
        '&V_EQUTYPE_CODE=' + typecode +
        '&V_EQU_CODE=' + equcode +
        '&V_DJ_PERCODE=' + djper +
        '&V_V_SHORT_TXT=' + Ext.getCmp("selshortTxt").getValue() +
        '&V_V_BJ_TXT=' + Ext.getCmp("selmatDesc").getValue() +
...
function unMatExcelButton() {
    ...
    var equcode = Ext.getCmp("sbmc").getValue() == '%' ? '0' : Ext.getCmp("sbmc").getValue();
    var djper = Ext.getCmp("djy").getValue() == '%' ? '0' : Ext.getCmp("djy").getValue();
    document.location.href = AppUrl + 'excel/WORKBJ_EXCEL?V_D_ENTER_DATE_B=' + Ext.Date.format(Ext.getCmp("begintime").getValue(), 'Y-m-d') +
        '&V_D_DEFECTDATE_E=' + Ext.Date.format(Ext.getCmp("endtime").getValue(), 'Y-m-d') +
        '&V_V_ORGCODE=' + orgcode +
        '&V_V_DEPTCODE=' + deptcode +
        '&V_V_DEPTCODEREPARIR=' + repaircode +
