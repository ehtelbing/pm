...
var V_PERSONNAME = Ext.util.Cookies.get("v_personname2");
...
Ext.onReady(function () {
    ...
    var gridStore = Ext.create('Ext.data.Store', {
        ...
        proxy: {
            ...
            url: AppUrl + 'YS/YS_CHARGE_WORKORDER_N_SEL',
            reader: {
                type: 'json',
                root: 'list',
                total:'total'
...
function _querySum(){
    Ext.Ajax.request({
        url: AppUrl + 'YS/YS_CHARGE_WORKORDER_TOTAL_SEL',
        async: false,
        method: 'POST',
        params: {
            'V_V_ORDERID': Ext.getCmp('workorder').getSubmitValue()
        },
...
function _chargeBill() {
    Ext.Ajax.request({
        url: AppUrl + 'YS/YS_CHARGE_WORKORDER_SET',
        method: 'POST',
        async: false,
        params: {
            'V_I_IP': GetIp(),
            'V_USERCODE': V_PERSONCODE,
            'V_USERNAME': V_PERSONNAME,
            'V_V_ORDERID': Ext.getCmp('workorder').getSubmitValue()
        ...
        success: function (response) {
            ...
            if (data.RET == 'success') {
                Ext.getCmp('statebill').setValue('未确认');
                _seek();
                _querySum();
            }else{
                Ext.Msg.alert("提示",data.RET);
            }
        }
    });
}
