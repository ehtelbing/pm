...
function OnButtonDefectAddClicked() {
    V_WEEKPLAN_GUID = 0;
    V_PLANTYPE = 'DEFECT';
    var ret = window.open(AppUrl + 'page/PM_03010313/index.html', '', 'height=600px,width=1200px,top=50px,left=100px,resizable=yes');
                ...
                if (resp.V_INFO == '成功') {
                    Ext.Ajax.request({
                        ...
                        params : {
                            ...
                            parVal : [Ext.util.Cookies.get('v_personcode'),records[i].get('V_GUID'),Ext.getCmp('nextPer').getValue(),"请审批!",records[i].get('V_CONTENT'),records[i].get('V_WEEKID'),"请审批！"],
