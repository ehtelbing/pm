...
function OnButtonUp() {
    ...
    //    fn: function (btn) {
    //        if (btn == 'yes') {
    //----update 20180907 start--------------
    var i_err = 0;
    for(var i=0;i<records.length;j++){
        if(records[i].data.V_STARTTIME<Ext.getCmp('starttime').getValue()){
            Ext.Msg.alert('提示', '存在“计划停工时间”小于“上报截止开始时间”！，请从新修改后上报');
        }
    else {
            Ext.Ajax.request({
