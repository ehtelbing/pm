...
function pageLoadInfo() {
    if (V_V_GUID == '0') {
        ...
        if (WEEK == null || WEEK == '') {
            Ext.getCmp('week1').setValue(getWeekOfMonth());
        } else {
            Ext.getCmp('week1').setValue(WEEK);
        }
    }
...
function OnButtonEditClicked() {
    ...
    // var ret = window.open(AppUrl + 'page/PM_03010310/index.html?V_WEEKPLAN_GUID=' + V_WEEKPLAN_GUID, '',
    //     'height=450px,width=650px,top=50px,left=100px,resizable=yes');
    Ext.getCmp('windowEqu').show();
    pageLoadInfo();
    if (V_V_GUID == '0') {
        V_JXGX_CODE = guid();
        ...
        Ext.Ajax.request({
            ...
            success: function (resp) {
                ...
                if (resp.list.length == 1) {
                    ...
                    Ext.getCmp('week1').select(V_WEEK);  //周
                    Ext.getCmp('ck1').select(V_ORGCODE);  //厂矿编码
                    Ext.getCmp('zyq1').select(V_DEPTCODE);  //作业区编码
                    Ext.getCmp('zy1').select(V_REPAIRMAJOR_CODE);  //专业编码
...
function jhSelect() {
    ...
    var ret = window.open(AppUrl + 'page/PM_03010316/index.html?V_V_YEAR=' + Ext.getCmp("nf").getValue()
...
function _close(){
    if (navigator.userAgent.indexOf("Firefox") != -1 || navigator.userAgent.indexOf("Chrome") !=-1) {
        window.location.href="http://localhost:8080/pm/app/pm/page/PM_03010301/index.html";
        window.close();
    } else {
        window.opener = null;
        window.open("", "_self");
