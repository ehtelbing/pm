/**
 * Created by hrb on 2019/6/19.
 * create workorder from weekplan
 */
...
if (location.href.split('?')[1] != undefined) {
    V_GUID = Ext.urlDecode(location.href.split('?')[1]).V_V_ORDERGUID;
    ...
    Ext.Ajax.request({
        ...
        success: function (ret) {
            ...
            if ($("#V_EQUIP_NO").val() == "") {
                ...
                var owidth = window.document.body.offsetWidth - 200;
                var oheight = window.document.body.offsetHeight - 100;
                var ret = window.open(AppUrl + 'page/PM_050102/index.html?flag=all&V_ORDERGUID=' + $("#V_ORDERGUID").val() + '&orderType='+ $("#selType").val(), '', '_blank',  'height=' + oheight + ',width=' + owidth + ',top=10px,left=10px,resizable=yes');
                loadMatList();
            }
        }
    });
