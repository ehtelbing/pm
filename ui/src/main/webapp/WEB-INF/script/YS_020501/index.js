...
Ext.onReady(function () {
    ...
    var gridStore = Ext.create('Ext.data.Store', {
        ...
        proxy: {
            ...
            url: AppUrl + 'YS/YS_CHARGE_WORKORDER_CON_SEL',
            reader: {
                type: 'json',
                root: 'list',
                total:'total'
    ...
    Ext.getCmp('stateBill').select('未确认');
...
function _querySum(){
    Ext.Ajax.request({
        url: AppUrl + 'YS/YS_CHARGE_W_CON_TOTAL_SEL',
        async: false,
        method: 'POST',
        params: {
...
function _confirm() {
    ...
    var V_V_CHARGE_ID_LIST = new Array();
    if (records.length == 0) {
        Ext.example.msg("提示", "没有选中数据");
    } else {
        Ext.Msg.confirm("提示", "结算单是否要确认？",
            function (button) {
                if (button === "yes") {
                    for (var i = 0; i < records.length; i++) {
                        V_V_CHARGE_ID_LIST.push(records[i].get('V_CHARGE_ID'));

                    }
                    Ext.Ajax.request({
                        url: AppUrl + 'YS/YS_CHARGE_WORKORDER_CON_SET',
                        method: 'POST',
                        async: false,
                        params: {
                            'V_V_CHARGE_ID_LIST': V_V_CHARGE_ID_LIST
                        },
                        success: function (resp) {
                            var data = Ext.decode(resp.responseText);
                            if (data.RET == 'success') {
                                Ext.getCmp('stateBill').setValue('%');
                                _seek();
                                _querySum();
                            }else {
                                Ext.Msg.alert("提示","结算单已确认！");
                            }
                        }
                    });
                }
