...
var gridStore = Ext.create('Ext.data.Store', {
    ...
    proxy : {
        type : 'ajax',
        async : false,
        url : AppUrl + 'Zyk/PRO_RUN7125_EQUVGLIST',
        actionMethods : {
            read : 'POST'
        },
        reader : {
...
function create() {
    Ext.Ajax.request({
        url: AppUrl + 'Zyk/PRO_RUN7125_ADDEQUVG_NEW',
        method : 'POST',
        params : {
        ...
        success : function(resp) {
            ...
            if(resp.RET =="设备编码或VG图ID不存在"){
                Ext.example.msg('操作信息','{0}','设备编码或VG图ID不存在');
            }else if(resp.RET =="该设备和VG图已关联"){
                Ext.example.msg('操作信息','{0}','该设备和VG图已关联');
...
function deleteRecord(equId, vgId) {
    Ext.Ajax.request({
        url: AppUrl + 'Zyk/PRO_RUN7125_DELEQUVG',
        async: false,
        method: 'POST',
        params: {
            V_EQU_ID: equId,
