    ...
    Ext.Ajax.request({
        ...
        success: function (ret) {
            ...
            if (resp.list != "" && resp.list != null) {
                ...
                loadTypelist(resp.list[0].V_ORDER_TYP);
            }
        }
    });
}

function loadTypelist(typ) {
    $.ajax({
        ...
        success : function(resp) {
            ...
            $.each(resp.list, function(index, item) {
                if(typ==item.ORDER_TYP){
                    result.push("<option value=\"" + item.ORDER_TYP + "\" selected='selected' >" + item.ORDER_TYP_TXT + "</option>");
                }else{
...
function orderAccept(){
    if(!confirm("请确认物料是否完成出库！")){return false;}
    else{
        OrderSave();
        orderaccept1();
    }
...
function p_query() {
    ...
    var owidth = window.document.body.offsetWidth - 200;
    var oheight = window.document.body.offsetHeight - 100;
    var ret = window.open(AppUrl + 'page/Basic/addperson.html?depart=99170208&classcode='+V_TEAMCODE, '', 'height=' + oheight + ',width=' + owidth + ',top=10px,left=10px,resizable=yes');
}
