...
var centerPanel = {
 	   ...
 	   layout:{
 	    ...
 	    items:[
 	       ...
 	       {xtype:'textfield', fieldLabel:'设备类型', id:'V_EQUTYPENAME',labelAlign:'right',grow: true, allowBlank: false,colspan:3,width:517,listeners:{change:OnChangeIt}},{xtype:'label',text:'*',style:'color:red'},
		       ...
		       {xtype:'gridpanel',
		    	   store: gridstore,
			        ...
			        columns:[
			                 ...
			                 {text:'特性值', dataIndex:'V_EQUTYPETXVALUE',field : {xtype : 'textfield',id:'txz'}, align:'center', renderer:AlignSZ,flex:1}
			                 ],dockedItems:[
			                               {xtype:'panel',frame:true, layout:'column',height:35, items:[
			                                                                      ...
			                                                                      {xtype:'checkbox', style:'margin:2px 0 0 10px',id:'updateTL'},{xtype:'label',text:'更新同类设备', style:'margin:6px 0 0 10px'}
			                                                                      ]}
			                               ]
			        }
		       ]
...
function firstStep(){
	Ext.Ajax.request({
		url:AppUrl+'pm_19/PRO_SAP_PM_EQU_P_COPY',
		method:'POST',
		params:{
		...
		success:function(resp){
			...
			Ext.Ajax.request({
				...
				success:function(resp){
					...
					Ext.getCmp('V_DATE_B').setValue(respObj.list[0].V_DATE_B==""?new Date():respObj.list[0].V_DATE_B);
					Ext.getCmp('V_DATE_E').setValue(respObj.list[0].V_DATE_E==""?"99991231":respObj.list[0].V_DATE_E);
...
function OnClickSaveButton(){
	if(!ValidateMes(limits)){
		Ext.example.msg('操作信息', '{0}', "请填写必填项后再保存！");
	}else{
		Ext.Ajax.request({
