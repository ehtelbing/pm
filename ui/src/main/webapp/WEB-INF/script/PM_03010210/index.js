    ...
    items: [
        ...
        {layout: 'column', defaults: {labelAlign: 'right'},frame:true,border: false,baseCls: 'my-panel-no-border',
            items: [
                ...
                {xtype: 'button', text: '导出Excel', margin: '10 0 5 10',icon: imgpath + '/grid.png',handler:OnButtonExcelClicked}
            ]
        }
    ]
});
...
function OnButtonExcelClicked(){
    document.location.href=AppUrl + 'excel/MSCBSP_EXCEL?V_V_INPER='+Ext.util.Cookies.get('v_personcode')+
                            '&V_V_YEAR='+Ext.getCmp('nf').getValue()+
                            '&V_V_MONTH='+Ext.getCmp('yf').getValue()+
                            '&V_V_ORGCODE='+Ext.getCmp('jhck').getValue()+
                            '&V_V_DEPTCODE='+ Ext.getCmp('jhzyq').getValue()+
