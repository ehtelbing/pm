...
Ext.onReady(function () {
    ...
    var gridStore = Ext.create('Ext.data.Store', {
        ...
        pageSize: 20,
        storeId: 'gridStore',
        fields: ['V_CHARGE_ID', 'V_ORDERGUID', 'V_ORDERID', 'V_ORGNAME', 'V_DEPTNAME', 'V_EQUIP_NAME', 'V_FUNC_LOC', 'V_TS', 'V_DE',
            'V_WORK_PER', 'V_CHARGE_PER','REPAIRNAME'],
        proxy: {
            type: 'ajax',
            actionMethods: {
                read: 'POST'
            },
            url: AppUrl + 'YS/YS_CHARGE_WORKORDER_PER_SEL',
            reader: {
                type: 'json',
                root: 'list',
                total:'total'
...
function _querySum(){
    Ext.Ajax.request({
        url: AppUrl + 'YS/YS_CHARGE_W_PER_TOTAL_SEL',
        async: false,
        method: 'POST',
        params: {
        ...
        success: function (resp) {
            ...
            if (data.RET=="success") {
                Ext.getCmp('SUM_PER').setValue(data.RET1);
            }
        }
    });
}
