							...
							click : function() {
								...
								if (id != '1') {
									...
									window
										.open(
										AppUrl
										+ 'page/No7102/Index.html?V_MPCODE='
							...
							click : function() {
								...
								if (id != '1') {
									...
									var paramUrl = AppUrl
										...
										+ selectedRecord.BJ_ID;
									var paramId = selectedRecord.BJ_ID;
									var paramSet = "dialogWidth=1000px;dialogHeight=400px";
									window.open(
										paramUrl,
										paramId,
										paramSet);
...
function OnButtonDeleteClicked() {
	...
	if (id == '0') {
		...
		for (i = 0; i < Ext.getCmp('grid').getSelectionModel().getSelection().length; i++) {
			...
			/*Ext.Ajax.request({
				url : APP + '/ModelChange',
				// '/No5103/pg_del_weekrepairplan',
				async : false,
				method : 'POST',
				params : {
					parName : [ 'A_BJ_ID' ],
					parType : [ 's' ],
					parVal : [ selectModel.getSelection()[i].data.BJ_ID ],
					proName : 'PRO_RUN_BJ_DELETE',
...
function winAddSave() {
		...
		success: function (ret) {
			...
			if(resp.RET=="Success"){
				Ext.example.msg('操作信息', '录入成功');
							Ext.getCmp('spareId_Add').setValue(''), Ext.getCmp(
								'spareMore_Add').setValue(''), Ext.getCmp(
								'normsType_Add').setValue(''), Ext.getCmp('unit_Add')
								.setValue(''), Ext.getCmp('remarks_Add').setValue(''),
								Ext.getCmp('winAdd').hide();
	...
	//	method : 'POST',
	//	params : {
	//		parName : [ 'A_BJ_ID', 'A_BJ_DESC', 'A_BJ_TYPE', 'A_BJ_UNIT',
	//			'A_BJ_REMARK', 'A_PLANTCODE', 'A_DEPARTCODE', 'A_EQUID','a_pre_flag' ],
	//		parType : [ 's', 's', 's', 's', 's', 's', 's', 's','s' ],
	//		parVal : [ Ext.getCmp('spareId_Add').getValue(),
	//			Ext.getCmp('spareMore_Add').getValue(),
	//			Ext.getCmp('normsType_Add').getValue(),
	//			Ext.getCmp('unit_Add').getValue(),
	//			Ext.getCmp('remarks_Add').getValue(),
	//			Ext.getCmp('selPlant').getValue(),
	//			Ext.getCmp('selSection').getValue(),
	//			Ext.getCmp('nowDevice_Id').getValue(),Ext.getCmp('pre_flag_Add').getValue() ],
	//		proName : 'PRO_RUN_BJ_ADD',
	//		returnStr : [ 'RET_MSG', 'RET' ],
	//		returnStrType : [ 's', 's' ]
	//	},
...
function winModifySave() {
	Ext.Ajax.request({
		...
		success: function (ret) {
			...
			if(resp.RET=="Success"){
				...
				Ext.getCmp('spareId_Modify').setValue(''), Ext.getCmp(
					'spareMore_Modify').setValue(''), Ext.getCmp(
					'normsType_Modify').setValue(''), Ext.getCmp(
					'unit_Modify').setValue(''), Ext.getCmp(
					'remarks_Modify').setValue(''), Ext.getCmp('winModify')
					.hide();
	...
	/*Ext.Ajax.request({
		...
		params : {
			parName : [ 'A_BJ_ID', 'A_BJ_DESC', 'A_BJ_TYPE', 'A_BJ_UNIT',
				'A_BJ_REMARK','a_pre_flag' ],
			parType : [ 's', 's', 's', 's', 's','s' ],
			parVal : [ Ext.getCmp('spareId_Modify').getValue(),
				Ext.getCmp('spareMore_Modify').getValue(),
				Ext.getCmp('normsType_Modify').getValue(),
				Ext.getCmp('unit_Modify').getValue(),
				Ext.getCmp('remarks_Modify').getValue(),
				Ext.getCmp('pre_flag_Modify').getValue()],
			proName : 'PRO_RUN_BJ_UPDATE',
			returnStr : [ 'RET_MSG', 'RET' ],
			returnStrType : [ 's', 's' ]
		},
		success : function(resp) {
