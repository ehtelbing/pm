...
var northPanel = Ext.create('Ext.form.Panel', {
    ...
    tbar: [
        ...
        { xtype: 'tbseparator',baseCls:'x-toolbar-separator-horizontal', margin:'3 8 5 8', hidden:true },
        {
            xtype: 'button',
            text: '临时保存',
            margin: '0 0 5 0',
...
var TOPGIRDRIGHTTtool = Ext.create('Ext.grid.Panel', {//form
    ...
    columns: [
        ...
        // {text: '通过数量',width: 140, dataIndex: 'PASSNUM', align: 'center',renderer:clickPass},
        // {text: '不通过数量',width: 140, dataIndex: 'NOPASSNUM', align: 'center',renderer:clickDisPass},
...
function QueryPageLoad(){
    ...
    Ext.Ajax.request({
        ...
        success: function (resp) {
            ...
            if(resp.list!=null){
                Ext.getCmp('northPanel').setTitle(resp.list[0].V_YEAR+"年"+resp.list[0].V_ORGNAME+"大修计划审批");
                ...
                if(resp.list[0].EQUNUM==0){
                    Ext.getCmp('Toolpanel').hide();
                    Ext.getCmp('addSbBtn').hide();
                }
                //计划类别
    ...
    Ext.Ajax.request({
        ...
        success: function (resp) {
            ...
            if(resp.V_INFO=='SUCCESS'){
                ...
                // //写入缺陷跟踪明细
                // Ext.Ajax.request({
                //     url:AppUrl+'dxfile/PRO_PM_07_DEFECT_LOG_SET',
                //     method:'POST',
                //     params:{
                //         V_V_GUID:records[i].get('V_GUID'),
                //         V_V_LOGREMARK:Ext.util.Cookies.get("v_personcode")+'审批维修计划（'+Guid+')中删除缺陷',
                //         V_V_FINISHCODE:'10',
                //         V_V_KEY:Guid
                //     },
                //     success:function (response){
...
function lcjsDefChange(){
    Ext.Ajax.request({
        url: AppUrl + 'dxfile/PM_DEFECT_LOG_FROMPRO_LCJS',
        method: 'POST',
        async: false,
        params: {
            V_PROGUID: Guid
...
function passDefect() {
    var records = Ext.getCmp("TOPGIRDRIGHTTtool").getSelectionModel().getSelection();
    for (var i = 0; i < records.length; i++) {
        var retdate= records[i].get('D_DEFECTDATE').split(" ")[0];
        var rethour=records[i].get('D_DEFECTDATE').split(" ")[1];
        var newdate=retdate.split("-")[0]+retdate.split("-")[1]+retdate.split("-")[2]+rethour.split(":")[0]+rethour.split(":")[1]+rethour.split(":")[2];
            ...
            success: function (resp) {
                ...
                if (resp.RET == 'SUCCESS') {
                    ...
                    Ext.Ajax.request({
                        ...
                        params:{
                            V_V_GUID:records[i].get('V_GUID'),
                            V_V_LOGREMARK:Ext.util.Cookies.get("v_personcode")+'审批维修计划（'+Guid+')结束缺陷通过',
                            V_V_FINISHCODE:'50',
                            V_V_KEY:Guid
                        },
                        success:function (response){
            ...
            success: function (resp) {
                ...
                if (resp.RET == 'SUCCESS') {
                    ...
                    Ext.Ajax.request({
                        ...
                        params:{
                            V_V_GUID:records[i].get('V_GUID'),
                            V_V_LOGREMARK:Ext.util.Cookies.get("v_personcode")+'审批维修计划（'+Guid+')结束缺陷不通过',
                            V_V_FINISHCODE:'10',
                            V_V_KEY:Guid
                        },
                        success:function (response){
...
//     metaData.style="text-align:center";
//     return '<a href="javascript:viewPDatail(\''+record.data.V_GUID+'\')">'+value+'</a>'
// }
// function clickDisPass(value, metaData, record){
//     metaData.style="text-align:center";
//     return '<a href="javascript:viewPDatail(\''+record.data.V_GUID+'\')">'+value+'</a>'
// }
...
function viewPDatail(value){
    var owidth = window.document.body.offsetWidth - 600;
    var oheight = window.document.body.offsetHeight - 100;
    window.open(AppUrl + 'page/PM_03020105/passDetail.html?guid=' +Guid + '&Defectguid=' + value+'&random=' + Math.random(), '', 'height=' + oheight + ',width=' + owidth + ',top=10px,left=10px,resizable=yes' );
}