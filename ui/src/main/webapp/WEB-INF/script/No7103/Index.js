...
function getReturnValue(ret){OnButtonCheckClicked
	var str =ret.split('^');
	Ext.getCmp('wzbm').setValue(str[0]);
	Ext.getCmp('wzmc').setValue(str[1]);
	Ext.getCmp('ggxh').setValue(str[4]);
	Ext.getCmp('jldw').setValue(str[2]);
	Ext.getCmp('dj').setValue(str[3]);
}
function OnButtonCheckClicked() {
	var returnValue = window.open(AppUrl
		+ 'page/No710301/Index.html?V_ORDERGUID=' + "", null,
