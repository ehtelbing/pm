...
Ext.onReady(function () {
    ...
    var gridStore = Ext.create('Ext.data.Store', {
        autoLoad: false,
        storeId: 'gridStore',
        fields: ['V_YEAR', 'V_TOTAL_A_PROJECT', 'V_TOTAL_P_PROJECT'],
        proxy: {
            type: 'ajax',
            actionMethods: {
                read: 'POST'
            },
            url: AppUrl + 'YS/YS_CHART_PROJECT_YEAR_SEL',
            reader: {
                type: 'json',
                root: 'list'
            },
            ...
            tips: {//为可视化标记添加工具栏
                ...
                renderer: function (storeItem, item) {
                    ...
                    if (item.yField == 'V_TOTAL_A_PROJECT') {
                        this.setTitle(item.value[0] + '年实际：' + item.value[1] + '元'/*Ext.util.Format.numberRenderer(item.value[1])*/);
                    } else {
                        this.setTitle(item.value[0] + '年计划：' + item.value[1] + '元'/*Ext.util.Format.numberRenderer(item.value[1])*/);
                    }
