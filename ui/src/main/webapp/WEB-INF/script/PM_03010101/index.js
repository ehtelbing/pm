...
var quarters=[{ displayField:'春季', valueField:'1'},{ displayField:'夏季', valueField:'2'},
	{ displayField:'秋季', valueField:'3'},{ displayField:'冬季', valueField:'4'}];
...
var nowmonth = Ext.Date.format(new Date(), 'Y-m-d').substring(5,7);
...
Ext.onReady(function () {
	...
	var gridStore = Ext.create('Ext.data.Store', {
		...
		proxy: {
			type: 'ajax',
			async: false,
			url: AppUrl + 'hp/PM_03_QUARTER_PLAN_SEL',
			actionMethods: {
				read: 'POST'
			},
			reader: {
	...
	var grid = Ext.create('Ext.grid.Panel', {
		...
		columns: [{xtype: 'rownumberer', text: '序号', width : 50,align:'center'},
			...
			{ text: '计划停工时间',width : 160,dataIndex: 'V_STARTTIME', align: 'center',
			...
			{ text: '计划竣工时间', width : 160, dataIndex: 'V_ENDTIME', align: 'center',
				renderer : function(value, metaData, record, rowIndex, colIndex, store, view) {//渲染
			...
			{text: '录入人', width : 100, dataIndex: 'V_INPERNAME', align: 'center'},
...
function OnButtonAddClicked() {
	var owidth = 593;
	var oheight = 796;
	// var ret = window.open(AppUrl + 'page/PM_03010112/index.html?V_YEARPLAN_GUID=0'
...
function _init1() {
    Ext.getCmp('jd1').select(getQuarterOfMonth());
    _gongshiheji();
    UPDATELOAD = true;
...
function OnButtonEditClicked() {
	...
	if (length != '1') {
		...
		var owidth = 600;
		var oheight = 400;
		// var ret = window.open(AppUrl + 'page/PM_03010113/index.html?V_V_GUID=' + V_V_GUID+'&UPDATE='
		// 	+update+'&YEAR='+ Ext.getCmp('grid').getSelectionModel().getSelection()[0].data.V_YEAR , '',
		// 	'height=796px,width=593px,top=50px,left=100px,resizable=yes');
        var yearStore = Ext.create("Ext.data.Store", {
            storeId: 'yearStore',
            fields: ['displayField', 'valueField'],
            data: years,
	...
	Ext.MessageBox.show({
		...
		fn: function (btn) {
			if (btn == 'yes') {
				...
				for(var i=0;i<records.length;i++) {
					Ext.Ajax.request({
						//url: AppUrl + 'hp/PRO_PM_03_PLAN_QUARTER_DEL',
...
function _close(){
    if (navigator.userAgent.indexOf("Firefox") != -1 || navigator.userAgent.indexOf("Chrome") !=-1) {
        window.location.href="http://localhost:8080/pm/app/pm/page/PM_03010101/index.html";
        window.close();
    } else {
        window.opener = null;
        window.open("", "_self");
...
function jhSelect(){
    ...
    var owidth = 1000;
    var oheight = 400;
    var ret = window.open(AppUrl + 'page/PM_03010116/index.html?V_V_YEAR=' +Ext.getCmp('year1').getValue()
        ...
        +'&V_V_JXNR='+Ext.getCmp('jxnr1').getValue()
...
function _save() {
    ...
    Ext.Ajax.request({
        ...
        params: {
            ...
            V_V_QUARTER: Ext.getCmp('jd1').getValue(),
            V_V_ORGCODE: Ext.getCmp('ck1').getValue(),
            V_V_DEPTCODE: Ext.getCmp('zyq1').getValue(),
