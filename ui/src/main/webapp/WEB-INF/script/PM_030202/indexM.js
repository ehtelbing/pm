...
//---create time :2018-10-12
    ...
    var jxdwstore = Ext.create("Ext.data.Store", {
        autoLoad: true,
        storeId: 'jxdwstore',
        fields: ['V_REPAIR_DEPTCODE', 'V_REPAIR_DEPTNAME'],
        proxy: {
            type: 'ajax',
            async: false,
    ...
    var leftpanel = Ext.create('Ext.grid.GridPanel', {
        id: 'leftpanel',
        region: 'west',
        height: 448,
        width: 280,
        ...
        //     checkOnly:true,
        //     listeners:{
        //         selected:function(model,record,index){
        //             chosvalue=record.get("V_DEPTREPAIRCODE");
        //             clickgrid;
        //         }
        //     }
        // }),
        columns: [
            {
                header: '编码', dataIndex: 'V_DEPTREPAIRCODE', width: '50%', listeners: {
                    'click':
                    clickgrid
                }
            },
            //         function(value, cellmeta, record, rowIndex, columnIndex, store){
            // chosvalue=store.data.V_DEPTREPAIRCODE;clickgrid(chosvalue)}}},
            {
                header: '名称', dataIndex: 'V_DEPTREPAIRNAME', width: '50%', listeners: {
                    'click':
                    clickgrid
                }
            }
    ...
    var midpanel = Ext.create('Ext.grid.GridPanel', {
        id: 'midpanel',
        region: 'center',
        width: 280,
        height: 448,
        ...
        columns: [
            {header: '编码', dataIndex: 'V_REPAIR_DEPTCODE', align: 'center', width: '33%'},
            {header: '名称', dataIndex: 'V_REPAIR_DEPTNAME', align: 'center', width: '33%'},
            {header: '操作', dateIndex: 'V_REPAIR_DEPTCODE', width: '33%',renderer:clickdel,listeners: { 'click':change} }
        ]
    ...
    Ext.Ajax.request({
        ...
        success: function (resp) {
            ...
            if (resp.list != null) {
                ...
                if (resp.list[0].V_TYPE == 'add') {
                    Ext.getCmp('agreeFlow').hide();
                    Ext.getCmp('zyq').setReadOnly(false);
                    Ext.getCmp('wxlx').setReadOnly(false);
                    Ext.getCmp('zy').setReadOnly(false);
                    Ext.getCmp('jhlb').setReadOnly(false);
                    Ext.getCmp('sclb').setReadOnly(false);
                    Ext.getCmp('sgfs').setReadOnly(false);
                    Ext.getCmp('sfxj').setReadOnly(false);
                    Ext.getCmp('cpzl').setReadOnly(false);
                    Ext.getCmp('cpgx').setReadOnly(false);
                } else {
                    Ext.getCmp('agreeFlow').show();
                    Ext.getCmp('zyq').setReadOnly(true);
                    Ext.getCmp('wxlx').setReadOnly(true);
                    Ext.getCmp('zy').setReadOnly(true);
                    Ext.getCmp('jhlb').setReadOnly(true);
                    Ext.getCmp('sclb').setReadOnly(true);
                    Ext.getCmp('sgfs').setReadOnly(true);
                    Ext.getCmp('sfxj').setReadOnly(true);
                    Ext.getCmp('cpzl').setReadOnly(true);
                    Ext.getCmp('cpgx').setReadOnly(true);
                }
                ...
                Ext.data.StoreManager.lookup('zyzxStore').on('load', function () {
                    if (resp.list[0].V_SPECIALTY_ZX == '') {
                        ...
                        var num = 0;
                        for (var i = 0; i < Ext.data.StoreManager.lookup('zyzxStore').data.items.length; i++) {
                            if (resp.list[0].V_SPECIALTY_ZX == Ext.data.StoreManager.lookup('zyzxStore').data.items[i].data.V_UID) {
                                num++;
                            }
                        }
                        if (num > 0) {
    ...
    Ext.Ajax.request({
        ...
        success: function (resp) {
            ...
            if (resp.V_INFO == '成功') {
                ...
                Ext.getCmp('WbsCode').setValue(resp.V_V_WBS);
            }
        }
    });
}
...
function LookMoreEqu() {
    ...
    window.open(AppUrl + 'page/PM_03020101/MoreEqu.html?guid=' + Guid + '&random=' + Math.random(), '', 'height=' + oheight + ',width=' + owidth + ',top=10px,left=10px,resizable=yes');
}
...
function btnYear() {
    var owidth = window.document.body.offsetWidth - 600;
    var oheight = window.document.body.offsetHeight - 100;
    window.open(AppUrl + 'page/PM_030202/indexY.html?guid=' + Guid + '&type=YEAR&Orgcode=' + OrgCode + '&Year=' + Year + '&random=' + Math.random(), '', 'height=' + oheight + ',width=' + owidth + ',top=10px,left=10px,resizable=yes');
}
...
function clickgrid(value, cellmeta, record, rowIndex, columnIndex, store) {
    chosvalue = store.data.V_DEPTREPAIRCODE;

    if (chosvalue != "" && chosvalue != 'undefined' && Ext.getStore('jxdwstore').data.length < 8) {
        Ext.Ajax.request({
            url: AppUrl + '/PM_03/PM_03_PLAN_REPAIR_DEPT_SET',
            type: 'ajax',
            async: false,
            method: 'POST',
            params: {
                V_V_GUID: Guid,
                V_V_REPAIR_DEPTCODE: store.data.V_DEPTREPAIRCODE,
                V_V_REPAIR_DEPTNAME: store.data.V_DEPTREPAIRNAME,
                V_V_TYPE: 'MONTH'
            },
            success: function (response) {
                if (Ext.decode(response.responseText).V_INFO == 'SUCCESS') {
        ...
        alert('每个月计划，最多只能添加8个检修单位');
    }

}

function clickdel(value, cellmeta, record, rowIndex, columnIndex, store) {
    cellmeta.style = 'text-align:center';
   // return '<a href=javascript:change(\'' + record.get('V_REPAIR_DEPTCODE') + '\')">' + "删除" + '</a>';
    return '<a href=#>' + "删除" + '</a>';
}

function change(value, cellmeta, record, rowIndex, columnIndex, store) {
    Ext.Ajax.request({
        url: AppUrl + '/PM_03/PM_03_PLAN_REPAIR_DEPT_DEL',
        type: 'ajax',
        async: false,
        method: 'POST',
        params: {
            V_V_GUID: Guid,
            V_V_REPAIR_DEPTCODE: store.data.V_REPAIR_DEPTCODE
        },
        success: function (response) {
            if (Ext.decode(response.responseText).V_CURSOR == 'SUCCESS') {
                Ext.data.StoreManager.lookup('jxdwstore').load({
                    params: {
                        V_V_GUID: Guid
                    }
