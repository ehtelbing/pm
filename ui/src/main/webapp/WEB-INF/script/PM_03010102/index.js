...
Ext.onReady(function () {
	...
	var grid = Ext.create('Ext.grid.Panel', {
		...
		columns: [{xtype: 'rownumberer', text: '序号', width : 50,align:'center'},
			...
			{text: '录入人', width : 50,  dataIndex: 'V_INPERNAME', align: 'center'},
...
function OnButtonEditClicked() {
	...
	if (length != '1') {
		...
		var ret = window.open(AppUrl + 'page/PM_03010113/index.html?V_V_GUID=' + V_V_GUID+'&UPDATE=' +update+'&YEAR='+ Ext.getCmp('grid').getSelectionModel().getSelection()[0].data.V_YEAR , '', 'height=796px,width=593px,top=50px,left=100px,resizable=yes');
	}
}
...
function OnButtonUpClicked() {
	...
	Ext.Ajax.request({
		url: AppUrl + 'hp/PRO_PM_03_PLAN_QUARTER_SEND',
		type: 'ajax',
		method: 'POST',
		params: {
			V_V_GUID: V_GUID,
...
function OnButtonExcelClicked() {
	...
	//var ss = '%';
	document.location.href = AppUrl + 'hp/PM_03_EXCEL_QUARTER?V_V_YEAR=' + Ext.ComponentManager.get("year").getValue()
		+ '&V_V_QUARTER=' + Ext.ComponentManager.get("jd").getValue()
