...
Ext.onReady(function () {
    ...
    var hChoGridStore = Ext.create('Ext.data.Store', {
        ...
        pageSize: 15,
        autoLoad: false,
        fields: ['D_DEFECTDATE', 'V_DEFECTLIST', 'V_EQUNAME', 'V_EQUCODE',
            'V_EQUSITE', 'V_DEPTNAME', 'V_PERNAME', 'V_IDEA',
            'V_STATENAME', 'V_SOURCENAME', 'V_SOURCEID',
            'D_INDATE', 'V_PERCODE', 'V_GUID', 'V_STATECODE',
            'V_STATECOLOR', 'V_ORDERID', 'V_EQUTYPECODE', 'V_SOURCECODE'],
...
function Select() {
    ...
    var hChoGridData = Ext.data.StoreManager.lookup("hChoGridStore").data.items;

    for (var i = 0; i < hChoGridData.length; i++) {
        if (hChoGridData[i].data.V_EQUCODE != equdef) {
            alert("请选择设备相同的缺陷");
            return false;
        }
    }
