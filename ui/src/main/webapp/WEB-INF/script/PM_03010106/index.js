...
var northPanel = Ext.create('Ext.form.Panel', {
	...
	items: [
		{layout: 'column', defaults: {labelAlign: 'right'},frame:true,border: false,baseCls: 'my-panel-no-border',
			items: [
				{xtype: 'combo',id:'nf',fieldLabel: '年份',editable: false, margin: '10 0 0 5',labelWidth:30,width:130,displayField: 'displayField',valueField: 'valueField',value:'',store:yearStore,queryMode: 'local'},
				{xtype: 'combo',id:'jd',fieldLabel: '季度',editable: false, margin: '10 0 0 20',labelWidth:30,width:130,displayField: 'displayField',valueField: 'valueField',value:'',store:quarterStore,queryMode: 'local'},
				{xtype: 'combo',id:'jhck',fieldLabel: '计划厂矿',editable: false, margin: '10 0 0 20',labelWidth:55,width:205, displayField: 'V_DEPTNAME',valueField: 'V_DEPTCODE',value: '',store:jhckStore,queryMode: 'local'},
				{xtype: 'combo',id:'jhzyq',fieldLabel: '作业区', editable: false,margin: '10 0 0 20',labelWidth:45,width:195, displayField: 'V_DEPTNAME',valueField: 'V_DEPTCODE',value: '',store:jhzyqStore,queryMode: 'local'},
				{xtype: 'combo',id:'zy',fieldLabel: '专业',editable: false, margin: '10 0 0 20',labelWidth:30,width:180, value: '',displayField: 'V_BASENAME',valueField: 'V_SPECIALTYCODE',store: zyStore,queryMode: 'local'}
			]
		},
		{layout: 'column', defaults: {labelAlign: 'right'},frame:true,border: false,baseCls: 'my-panel-no-border',
			items: [
				...
				{xtype: 'button', text: '通过', margin: '10 0 5 10',icon: imgpath + '/saved.png',handler:OnButtonPassClicked},
				{xtype: 'button', text: '驳回', margin: '10 0 5 10',icon: imgpath + '/cross.png',handler:OnButtonBackClicked}
			]
		}
	]
});
...
var gridPanel= Ext.create('Ext.grid.Panel', {
	...
	columns:[
		...
		{text: '录入时间', align: 'center', width: 180, dataIndex: 'V_INDATE',renderer: Ext.util.Format.dateRenderer('Y-m-d H:i:s')}
	],
	bbar:["->",
		{
			xtype: 'pagingtoolbar',
...
function OnButtonAddClicked() {
	V_QUARTERPLAN_GUID = '0';
	var ret = window.open(AppUrl + 'page/PM_03010112/index.html?V_QUARTERPLAN_GUID=' + V_QUARTERPLAN_GUID +
...
function OnButtonEditClicked(){
	...
	V_QUARTERPLAN_GUID=seldata[0].data.V_QUARTERPLAN_GUID;
	var ret = window.open(AppUrl + 'page/PM_03010112/index.html?V_QUARTERPLAN_GUID='+V_QUARTERPLAN_GUID,'','height=600px,width=1200px,top=50px,left=100px,resizable=yes');
}
//删除
function OnButtonDelete(){
	...
	for(var i=0;i<length;i++){
		Ext.Ajax.request({
			url: AppUrl + 'PM_03/PRO_PM_03_PLAN_QUARTER_DEL',
			method: 'POST',
			async : false,
			params:{
				V_V_QUARTERPLAN_GUID : Ext.getCmp('gridPanel').getSelectionModel().getSelection()[i].data.V_QUARTERPLAN_GUID
			},
