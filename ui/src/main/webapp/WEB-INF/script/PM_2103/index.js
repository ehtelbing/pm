...
var zyStore = Ext.create('Ext.data.Store', {
    ...
    proxy: {
        type: 'ajax',
        async: false,
        url: AppUrl + 'basic/PRO_BASE_SPECIALTYp_DEPT_SPECIN',
        actionMethods: {
            read: 'POST'
        },
        reader: {
...
function AgreeData() {
    ...
    if (Ext.getCmp('tabid').getValue() == 'WeekPlan') {
    ...
    }else if (Ext.getCmp('tabid').getValue() == 'YearPlan') {
        var record = Ext.getCmp('grid').getSelectionModel().getSelection();
        var num = 0;
        var fnum = 0;
        if (record.length > 0) {
            ...
            Ext.Ajax.request({
                url: AppUrl + 'cjy/batchAgreeForYear',
                async: false,
                type: 'ajax',
                method: 'POST',
                params: {
        ...
        if (record.length > 0) {
            ...
            Ext.Ajax.request({
                url: AppUrl + 'cjy/batchAgreeForMaintain',
                async: false,
                type: 'ajax',
                method: 'POST',
                params: {
...
function DisAgreeData() {
    ...
    if (Ext.getCmp('tabid').getValue() == 'WeekPlan') {
        ...
        if (record.length > 0) {
            ...
            Ext.Ajax.request({
                url: AppUrl + 'cjy/batchDisAgreeForYear',
                async: false,
                type: 'ajax',
                method: 'POST',
                params: {
