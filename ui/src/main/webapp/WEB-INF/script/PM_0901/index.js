    ...
    Ext.Ajax.request({
        ...
        success: function (ret) {
            ...
            if ($("#V_EQUIP_NO").val() == "") {
                ...
                /*var ret = window.open(AppUrl + 'page/PM_050102/index.html?flag=all&V_ORDERGUID=' + $("#V_ORDERGUID").val() + '&V_EQUCODE='+$("#V_EQUCODE").val()+'&DEPTCODEREP='+$("#selPlant").val()
                    +'&STEP='+document.title+'','_blank','height=' + oheight + ',width=' + owidth + ',top=10px,left=10px,resizable=yes');*/
                var ret = window.open(AppUrl + 'page/PM_050104/index.html?flag=all&V_ORDERGUID=' + $("#V_ORDERGUID").val() + '&V_EQUCODE='+$("#V_EQUCODE").val()+'&DEPTCODEREP='+$("#selPlant").val()
                    +'&STEP='+document.title+'','_blank','height=' + oheight + ',width=' + owidth + ',top=10px,left=10px,resizable=yes');
                loadMatList();
            }
        }
