    ...
    var faultItemPanel = Ext.create('Ext.grid.Panel', {
        ...
        columns: [{
            ...
            renderer: function (value, metaData, record, rowIdx, colIdx, store, view) {
                return '<a href="#" onclick="_glFault(\'' +record.data.V_GUID+ '\')">关联故障</a>';
            }
        }, {
            text: '状态',
    ...
    Ext.ComponentManager.get('foundview')
        .on(
            ...
            function(view, rowIndex, colIndex, item, e) {
                ...
                window.open(AppUrl + "page/LowFault_1405/workorderlist.html?faultguid=" + data.V_GUID,
...
function _addOpen() {
    Ext.Ajax.request({//是否已关联故障
        url: AppUrl + 'cxy/PM_WORKORDER_BUG_GL_SEL',
        type: 'post',
        async: false,
        params: {
        ...
        success: function (response) {
            ...
            if (resp.num==0) {
                ...
                window.open(AppUrl + 'page/LowFault_1405/workordertohitch.html?V_V_ORGCODE='
...
function _deleteFaultGL() {
    Ext.MessageBox.show({
        ...
        fn: function (btn) {
            if (btn == 'yes') {
                Ext.Ajax.request({
                    url: AppUrl + 'cxy/PM_WORKORDER_BUG_GL_DEL',
                    type: 'ajax',
                    method: 'POST',
                    params: {
                        'V_WORKORDER_GUID': v_workorder_guid,
