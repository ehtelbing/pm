...
Ext.onReady(function() {
	...
	var gridStore = Ext.create('Ext.data.Store',
			{
				storeId : 'gridStore',
				autoLoad : false,
				fields : [ 'SERIES_CLASS', 'SERIES_CLASS_DESC', 'SERIES_CLASS_STATUSDESC',
				           'SERIES_CLASS_STATUS'],
				proxy : {
					type : 'ajax',
					url : AppUrl + 'DJ/pro_dj106_selectdjseries',
					actionMethods : {
						read : 'POST'
					},
					reader : {
	...
	var grid = Ext.create('Ext.grid.Panel', {
		...
		columns : [ {
			...
			text : '状态',
			align : 'center',
			dataIndex : 'SERIES_CLASS_STATUSDESC',
			width : 80
		}, {
			text : '启用',
			align : 'center',
...
function OnStartButtonClicked(){
	globl_code =Ext.getCmp("grid").getSelectionModel().getSelection()[0].data.SERIES_CLASS;


	Ext.Ajax.request({
		url: AppUrl + 'DJ/pro_dj106_startdjseries',
		method : 'post',
		async:false,
		params : {
			series_class_in:globl_code
		},
...
function OnStopButtonClicked(){
	...
	Ext.Ajax.request({
		url: AppUrl + 'DJ/pro_dj106_stopdjseries',
		method : 'post',
		async:false,
		params : {
			series_class_in:globl_code
...
function OnClickSubmitAdd() {
	if (Ext.getCmp('xl_code').getValue() == '') {
		...
		Ext.Ajax.request({
			url: AppUrl + 'DJ/pro_dj106_adddjseries',
			method : 'post',
			async:false,
			params : {
				series_type_in:Ext.getCmp('xl_code').getValue(),
...
function OnClickSubmitDelete() {
	...
	Ext.Ajax.request({
		url: AppUrl + 'DJ/pro_dj106_deletedjseries',
		method : 'post',
		async:false,
		params : {
			series_class_in:globl_code
...
function renderer_start(value, metaData, record, rowIdx, colIdx, store, view) {
	var status =record.data.SERIES_CLASS_STATUSDESC;
	if (status == '停用'){
	return "<img src='"
			+ imgpath
