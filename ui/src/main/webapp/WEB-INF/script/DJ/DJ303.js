...
Ext.onReady(function() {
		     ...
		     {frame : true,style:'margin-bottom:1px',
		      ...
		      items:[
		          {id : 'plant', xtype : 'combobox', fieldLabel : '所属厂矿',editable :false,
			        queryMode: 'local',labelAlign :'right',displayField: 'V_DEPTNAME', valueField: 'V_DEPTCODE',
			        store: plantStore
				  },
				  ...
				  {bodyStyle:'background: none' ,border:0, items:[
				     ...
				     {xtype:'button',text:'导出Excel',id:'exp',margin:'0px 0px 0px 10px',icon:imgpath+'/311.gif',width:100}
				  ]}
		     ] }
		]
	});
	...
	plantStore.on('load',function(){
		plantStore.insert(0,{V_DEPTCODE:'%',V_DEPTNAME:'全部'});
		plantStore.sort('V_DEPTCODE','ASC');

		Ext.getCmp('plant').select(plantStore.getAt(0));

	});
	deptStore.on('load',function(){
		deptStore.sort('V_DEPTCODE','ASC');
	...
	Ext.getCmp('dept').on('select',query);
	...
	function query(){ gridStore.load(); }
	  ...
	  Ext.getCmp('plant').on('select',queryDeptList);
