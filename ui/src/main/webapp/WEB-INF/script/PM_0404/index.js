...
var poststore = Ext.create("Ext.data.Store",{
    ...
    proxy : {
        type : 'ajax',
        url : APP + '/BaseRepository/post_Sel',
        async : false,
        actionMethods : {
            read : 'POST'
        },
    ...
    columns: [
        {
            ...
            text: '考核单位',
            dataIndex: 'V_KH_DEPTNAME',
            align: 'center',
            width: 120
        }, {
            text: '考核职别',
            dataIndex: 'V_KH_POSTNAME',
            align: 'center',
            width: 100
        },{
            text: '考核责任人',
            dataIndex: 'V_KH_PERCODE',
            align: 'center',
            width: 100
        }, {
            text: '详细',
...
function infosave(){
    ...
    Ext.Ajax.request({
        url: AppUrl + 'sg/SG_KH_INF_ITEM_EDIT',
        method: 'POST',
        async: false,
        params: {
            V_V_ID : v_id,
            V_V_GUID : guid,
            V_V_ORG : Ext.getCmp('entrykhdw').getValue(),
            V_V_ZB :  Ext.getCmp('entrykhjb').getValue(),
            V_V_PER :  Ext.getCmp('entrykhzrr').getValue(),
            V_V_TK : Ext.getCmp('entrykhtk').getValue(),
            V_V_BZ : Ext.getCmp('entrykhbz').getValue()
        },
        success: function (ret) {
            var resp = Ext.JSON.decode(ret.responseText);
        }
...
function onDetail(){
    ...
    Ext.Ajax.request({
        ...
        success: function (ret) {
            ...
            Ext.getCmp('entrykhdw').setValue(resp.list[0].V_KH_DEPTNAME);
            Ext.getCmp('entrykhjb').setValue(resp.list[0].V_KH_POSTNAME);
            Ext.getCmp('entrykhzrr').setValue(resp.list[0].V_KH_PERCODE);
            Ext.getCmp('entrykhtk').setValue(resp.list[0].V_KH_TK);
            Ext.getCmp('entrykhbz').setValue(resp.list[0].V_KH_MARK);
...
function uploadfile(){
    ...
    if (Ext.getCmp('upload').getValue() != "") {
        ...
        Ext.getCmp('V_V_FILETYPE').setValue("考核");
