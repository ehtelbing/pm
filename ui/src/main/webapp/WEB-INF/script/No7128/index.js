...
var gridStore = Ext.create("Ext.data.Store", {
    ...
    proxy : {
        type : 'ajax',
        async : false,
        url : AppUrl + 'lx/pro_run7128_junkmatlist',
        actionMethods : {
            read : 'POST'
        },
        reader : {
...
Ext.onReady(function() {
    ...
    Ext.getCmp('clzt').on('select',function(){
        if(Ext.getCmp('clzt').getValue()=='1'){
            Ext.getCmp('clxzjj').setDisabled(true);
        } else {Ext.getCmp('clxzjj').setDisabled(false);}
    });
    ...
    Ext.data.StoreManager.lookup('gridStore').load(
        {
            params : {
                ...
                V_MATERIALCODE : Ext.getCmp('wlh').getValue(),
...
function saved(){
    ...
    Ext.Ajax.request({
        url : AppUrl + 'lx/pro_run7129_junkmat',
        method : 'POST',
        async : false,
        params : {
...
function OnButtonExportClicked() {
    ...
    var D_BEGIN_DATE = Ext.util.Format.date(Ext.getCmp('startTime').getValue(), 'Y-m-d');
    var D_END_DATE = Ext.util.Format.date(Ext.getCmp('endTime').getValue(), 'Y-m-d');
    var V_PLANTCODE = ckcode;
    var V_DEPARTCODE = code;
    var V_EQU_ID = Ext.getCmp('xzsb').getValue();
    var V_MATERIALCODE = Ext.getCmp('wlh').getValue();
    var V_MATERIALNAME = Ext.getCmp('wlms').getValue();
    var V_STATUS = Ext.getCmp('clzt').getValue();

    document.location.href = AppUrl + 'lx/PRO_RUN7128_JUNKMATLIST_EXCLE?D_BEGIN_DATE='+ D_BEGIN_DATE +
        '&D_END_DATE='+ D_END_DATE +
        '&V_PLANTCODE=' + encodeURI(encodeURI(V_PLANTCODE)) +
        '&V_DEPARTCODE=' + encodeURI(encodeURI(V_DEPARTCODE)) +
        '&V_EQU_ID=' + encodeURI(encodeURI(V_EQU_ID)) +
        '&V_MATERIALCODE=' + encodeURI(encodeURI(V_MATERIALCODE)) +
        '&V_MATERIALNAME=' + encodeURI(encodeURI(V_MATERIALNAME)) +
        '&V_STATUS=' + encodeURI(encodeURI(V_STATUS)) ;
...
function loadgridStore(store){
        ...
        store.proxy.extraParams.V_PLANTCODE = ckcode;
        store.proxy.extraParams.V_DEPARTCODE = code;
