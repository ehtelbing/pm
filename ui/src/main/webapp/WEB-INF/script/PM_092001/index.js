...
$(function() {
    ...
    $("#btnTask").click(function () {
        /*if($("#V_MMCODE").val()==""||$("#V_MMCODE").val()==null||$("#V_MMNAME").val()==""||$("#V_MMNAME").val()==null||$("#V_NUM").val()==""||$("#V_NUM").val()==null){
            Ext.MessageBox.alert('提示', '请填写物资向关系');
            return false;
        }*/
        var owidth = window.document.body.offsetWidth-200;
        var oheight = window.document.body.offsetHeight-100 ;
...
function loadPageInfo() {
    Ext.Ajax.request({
            url: AppUrl + 'hp/PM_WORKORDER_OLD_EDIT',
            type : 'post',
            async : false,
            params : {
                V_V_GUID :  V_GUID
            ...
            success : function(response) {
                ...
                if (resp.list != "" && resp.list != null) {
                    ...
                    $("#V_MMCODE").html(resp.list[0].V_MMCODE);
                    $("#V_MMNAME").html(resp.list[0].V_MMNAME);
                    $("#V_NUM").html(resp.list[0].V_NUM);
...
function CreateBill() {
    ...
    if ($("#V_DEFECTLIST").val() == '') {
        ...
        if (!confirm("确定下达工单?")) {
            ...
            Ext.Ajax.request({
                url : AppUrl + 'hp/PM_WORKORDER_OLD_SAVE',
                type : 'post',
                async : false,
                params : {
