...
Ext.onReady(function() {
	...
	var grid = Ext.create('Ext.grid.Panel', {
		...
		store:gridStore,
		autoScroll: true,
		dockedItems:[{xtype:'panel',style: 'background-color:#FFFFFF',
			baseCls: 'my-panel-no-border',defaults:{labelAlign:'right',labelWidth:85},baseCls:'my-panel-no-border',layout:'column',bodyPadding:3,items:[
	...
	Ext.Ajax.request({
		...
		success: function (ret) {
			...
			if(resp.RET=="Success"){
				Ext.ComponentManager.get('dqbjms').setValue(resp);
			}else {
			}
		}
	});
