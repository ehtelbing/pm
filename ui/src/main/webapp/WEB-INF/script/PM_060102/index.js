...
function _insertDjDataYC(str) {
    ...
    Ext.Ajax.request({
        ...
        params: {
            ...
            'V_V_DJ_PER': Ext.util.Cookies.get('v_personcode'),
            'V_V_DJ_NR': V_MS,
            'V_V_DJ_TYPE':'GW'//Ext.getCmp('djTypeSelect').getValue()
        },
        success: function (response) {
            var data = Ext.decode(response.responseText);
            if (data.success) {
...
function detail(a,value,metaData){
    return '<a href="javascript:ondetail(\'' + metaData.data.V_GUID + '\')">详情</a>&nbsp;<input type="image" onclick="_djDataZCShow()" src="../../images/gif/saved.png">&nbsp;<input type="image" onclick="_djDataYCShow()" src="../../images/gif/tree_dnd_no.png">';
}
