...
function OpenEditMat() {
    Ext.Ajax.request({
        ...
        success: function (ret) {
            ...
            var owidth = window.document.body.offsetWidth - 200;
            var oheight = window.document.body.offsetHeight - 100;
            var ret = window.open(AppUrl + 'page/PM_050104/index.html我?flag=all&V_ORDERGUID=' + V_ORDERGUID + '', '', '_blank', 'height=' + oheight + ',width=' + owidth + ',top=10px,left=10px,resizable=yes');
            loadMatList();
        }
    });
}
...
function activitiOrderissued() {
    ...
    Ext.Ajax.request({
        ...
        params: {
            ...
            parVal: [choicePer, '已反馈'],//[Ext.getCmp('nextSpr2').getValue(), '已反馈'],
            processKey: processKey,
            businessKey: $.url().param("V_ORDERGUID"),
            V_STEPCODE: V_STEPCODE,
            V_STEPNAME: V_STEPNAME,
            V_IDEA: '已反馈',
            V_NEXTPER: choicePer,//Ext.getCmp('nextSpr2').getValue(),
