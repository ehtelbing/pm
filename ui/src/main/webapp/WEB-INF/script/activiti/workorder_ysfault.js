...
$(function () {
    ...
    var glwindow = Ext.create('Ext.window.Window', {
        id: 'glwindow',
        width: 400,
        height: 150,
        bodyPadding: 15,
        ...
        items: [{
            ...
            items: [ {
                    ...
                    text: '关联事故',
                    // icon: imgpath + '/saved.png',
                    style: ' margin: 10px 5px 10px 25px',
                    handler: toFault
                },
                {
                    xtype: 'button',
        ...
        success: function (response) {
            ...
            if (resp.ret == 'success') {
                ...
                if (date.length > 0) {
                    ...
                    Ext.getBody().unmask();
                    Ext.MessageBox.alert('提示', '没有对应数据不能验收');
                    return false;
                }
            }
        }
...
function OnClickJJButton() {
    ...
    var owidth = window.document.body.offsetWidth - 200;
    var oheight = window.document.body.offsetHeight - 100;
    var ret = window.open(AppUrl + 'page/PM_070204/index.html?V_ORDERGUID=' + $("#V_ORDERGUID").val()
...
function CreateModel() {
    var owidth = window.document.body.offsetWidth - 200;
    var oheight = window.document.body.offsetHeight - 100;
    var ret = window.open(AppUrl + 'page/FAULT_GZ/CreateModel.html?V_ORDERGUID=' + $("#V_ORDERGUID").val() +
...
function toDefect() {
    var owidth = window.document.body.offsetWidth - 200;
    var oheight = window.document.body.offsetHeight - 100;
    var ret = window.open(AppUrl + 'page/FAULT_1405/defect.html?v_workorder_guid=' + $("#V_ORDERGUID").val(), '', 'height=' + oheight + ',width=' + owidth + ',top=10px,left=10px,resizable=yes');
    // loadMatList();
}

function toFault() {
    Ext.Ajax.request({
        url: AppUrl + 'cxy/PM_WORKORDER_GL_STATUS_SEL',
        type: 'ajax',
        method: 'POST',
        params: {
            'V_V_ORDERGUID': $("#V_ORDERGUID").val()
        ...
        success: function (response) {
            var resp = Ext.JSON.decode(response.responseText);
            if(resp.V_INFO=='1'||resp.V_INFO=='0'){
                var owidth = window.screen.availWidth;
                var oheight = window.screen.availHeight;
                window.open(AppUrl + 'page/FAULT_1405/index2019.html?V_V_ORGCODE='//workordertofault
                    + V_V_ORGCODE+'&V_V_DEPTCODE='+V_V_DEPTCODE+'&v_workorder_guid=' + $("#V_ORDERGUID").val()
                    +'&begintime='+encodeURI($("#D_START_DATE").html())
            ...
            }else if(resp.V_INFO=='2'){
                Ext.Msg.alert('提示', '此工单已关联故障，请先删除故障关联');
            }
...
function toHitch() {
    Ext.Ajax.request({
        ...
        success: function (response) {
            var resp = Ext.JSON.decode(response.responseText);
            if(resp.V_INFO=='2'||resp.V_INFO=='0'){
                var owidth = window.screen.availWidth;
                var oheight = window.screen.availHeight;
                var owidth = window.screen.availWidth;
                var oheight = window.screen.availHeight;
                window.open(AppUrl + 'page/LowFault_1405/index2019.html?V_V_ORGCODE='
                    + V_V_ORGCODE+'&V_V_DEPTCODE='+V_V_DEPTCODE+'&v_workorder_guid=' + $("#V_ORDERGUID").val()
                    +'&begintime='+encodeURI($("#D_START_DATE").html())
                    /*+'&V_V_EQUTYPE='+encodeURI(Ext.getCmp('V_V_EQUTYPE').getValue())
                    +'&V_EQUNAME='+encodeURI(Ext.getCmp('V_EQUNAME').getValue())
            ...
            }else if(resp.V_INFO=='1'){
                Ext.Msg.alert('提示', '此工单已关联事故，请先删除事故关联');
            }
