...
if (location.href.split('?')[1] != undefined) {
    DEFGUID = Ext.urlDecode(location.href.split('?')[1]).DEFGUID;
...
$(function () {
    Ext.Ajax.request({
        url:AppUrl+'WorkOrder/PRO_WORKDET_BY_DEFEECT',
        method:'POST',
        async: false,
        params: {
            'DEFGUID': DEFGUID
        ...
        success:function(resp){
            ...
            if(res.list.length==1){
                V_ORDERGUID=res.list[0].V_WORKORDER_GUID;
            }
        }
    });
    ...
    $.ajax({
        ...
        success: function (resp) {
            if (resp.list == "" || resp.list == null) {
                ...
                var ret = window.open(AppUrl + 'page/PM_091104/index.html?V_ORDERGUID=' + V_ORDERGUID+ '&V_DEPTCODEREPARIR=' + V_V_DEPTCODEREPARIR, '', 'height=' + oheight + ',width=' + owidth + ',top=10px,left=10px,resizable=yes');
                loadTaskGrid();
            }
        }
    });
...
function loadPageInfo() {
    ...
    $.ajax({
        ...
        async: false,
        data: {
            V_V_ORDERGUID: V_ORDERGUID,//$.url().param("V_ORDERGUID")
        },
        dataType: "json",
        traditional: true,
        success: function (resp) {
...
function comboConfirm() {
    ...
    if (Ext.getCmp('radiotypesc').getValue().sctypename == '1') { //生成新的缺陷
        var workguid=V_ORDERGUID;//$.url().param("V_ORDERGUID");
            ...
            $.ajax({
                ...
                success: function (resp) {
                    ...
                    Ext.Ajax.request({
                        ...
                        params:{
                            V_WORKORDER:V_ORDERGUID,//$.url().param("V_ORDERGUID"),
                    ...
                    $.ajax({
                        ...
                        success: function (resp) {
                            ...
                            Ext.Ajax.request({// 四、查找工单对应的缺陷
                                ...
                                params: {
                                    V_V_WORKORDER_GUID: V_ORDERGUID,//$.url().param("V_ORDERGUID"),
                                    V_V_FLAG: "1"
                                },
                                success: function (response) {
                                    var respl = Ext.decode(response.responseText);
                                        ...
                                        for (var i = 0; i < respl.list.length; i++) {
                                            ...
                                            if( $("#V_WBS").html()!=""){
                                                ...
                                                Ext.Ajax.request({ //检修完成结果下传
                                                    ...
                                                    async: false,
                                                    params: {
                                                        V_V_WORKORDER: V_ORDERGUID,//$.url().param("V_ORDERGUID"),
                                                        V_V_DEFECT_TYPE:'1'
                                                    },
                                                    success: function (ret) {
                                                        var resp=Ext.decode(ret.responseText);
        ...
        if ($("#V_REPAIRSIGN").val() == "这个判断不执行") {
            ...
            if (!confirm("是否验收工单")) {
                ...
                $.ajax({
                    ...
                    success: function (resp) {
                        // 聚进接口
                        otherServer(V_ORDERGUID,/*$("#V_ORDERGUID").val()*/ "CLOSE", "成功");
                        // 小神探接口
                        xstServer(V_ORDERGUID,/*$("#V_ORDERGUID").val()*/ "CLOSE", "成功");
                        Ext.Msg.alert('提示', '验收工单成功');
                        window.opener.QueryTab();
                        window.opener.QuerySum();
                        window.opener.QueryGrid();
...
function print() {
    ...
    if(MATSIGN==1||returnMatSign=="1"){
        ...
        selectID.push(V_ORDERGUID);// $("#V_ORDERGUID").val()
...
function OnBtnHistoryClicked() {
    var ret = window.open(AppUrl + 'page/No41030101/Index.html?V_ORDERGUID=' +  V_ORDERGUID,/*$("#V_ORDERGUID").val(),*/ '', 'dialogHeight:500px;dialogWidth:800px');
    if (ret != "" && ret != null) {
        var str = [];
        str = ret.split('^');
...
function OnClickJJButton() {
    ...
    var owidth = window.document.body.offsetWidth - 200;
    var oheight = window.document.body.offsetHeight - 100;
    var ret = window.open(AppUrl + 'page/PM_090510/index.html?V_ORDERGUID=' +  V_ORDERGUID//,$("#V_ORDERGUID").val()
...
function CreateModel() {
    var owidth = window.document.body.offsetWidth - 200;
    var oheight = window.document.body.offsetHeight - 100;
    var ret = window.open(AppUrl + 'page/PM_091105/index.html?V_ORDERGUID=' + V_ORDERGUID+// $("#V_ORDERGUID").val() +
...
function OnBtnLookClicked() {
    var owidth = window.document.body.offsetWidth - 200;
    var oheight = window.document.body.offsetHeight - 100;
    var ret = window.open(AppUrl + 'page/PM_091107/index.html?V_ORDERGUID=' +  V_ORDERGUID,
...
function loadMatList() {
    Ext.Ajax.request({
        ...
        type: 'post',
        params: {
            V_V_ORDERGUID:  V_ORDERGUID,//$.url().param("V_ORDERGUID")// $("#V_ORDERGUID").val()
        },
        success: function (ret) {
            var resp = Ext.JSON.decode(ret.responseText);
