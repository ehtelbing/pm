...
var sbxzStore = Ext.create('Ext.data.Store', {
	autoLoad : false,
	storeId : 'sbxzStore',
	fields : [ 'V_EQUNAME', 'V_EQUCODE' ],
	proxy : {
		type : 'ajax',
		async : false,
		url: AppUrl + 'WorkOrder/PRO_RUN7111_EQULIST',
		actionMethods : {
			read : 'POST'
		},
		reader : {
...
var siteStore = Ext.create('Ext.data.Store', {
	...
	proxy : {
		type : 'ajax',
		async : false,
		url: AppUrl + 'WorkOrder/PRO_RUN_BJ_CHANGE_LOG_ALL',
		actionMethods : {
			read : 'POST'
		},
		reader : {
...
var supplyStore = Ext.create('Ext.data.Store', {
	...
	proxy : {
		type : 'ajax',
		async : false,
		url: AppUrl + 'WorkOrder/pro_run7110_sitesupplylist',
		actionMethods : {
			read : 'POST'
		},
		reader : {
...
var equBarGrid = Ext.create('Ext.grid.Panel', {
	...
	store : {
		...
		proxy : {
			type : 'ajax',
			async : false,
			url: AppUrl + 'WorkOrder/pg_run7113_getordermatbarcode',
			actionMethods : {
				read : 'POST'
			},
			reader : {
	...
	listeners : {
		'cellclick' : function(grid, rowIndex, columnIndex, e) {
			...
			else{var rec = Ext.data.StoreManager.lookup('siteStore').getAt(siteFlag);
...
var grid = Ext.create('Ext.grid.Panel',{
					...
					store : {
						...
						proxy : {
							type : 'ajax',
							async : false,
							url: AppUrl + 'WorkOrder/pro_run7113_ordermatlist',
							actionMethods : {
								read : 'POST'
							},
							reader : {
											...
											handler : function() {
												if (Ext.getCmp('jcnr').getValue() == '') {
													...
													if (Ext.getCmp('upload').getValue() != null|| Ext.getCmp('upload').getValue() != "") {
														...
														form.submit({url : AppUrl+ 'fileload/uploadImageFile',
																	async : false,
																	waitMsg : '上传中...',
																	params : {
...
Ext.onReady(function() {
			...
			Ext.data.StoreManager.lookup('zyqStore').on('load',function() {
				...
				// 默认当前登录用户工作区
				var storeLength = Ext.data.StoreManager.lookup('zyqStore').data.length;
				for ( var index = 0; index < storeLength; index++) {
					var storeItemData = Ext.data.StoreManager.lookup('zyqStore').data.items[index].data;
			...
			sbxzStore.on('load', function(me) {
				var index = Ext.data.StoreManager.lookup('sbxzStore').findBy(function(record, id) {
					return record.get('V_EQUCODE')==equid;
				});
				if(equid==''||index==-1){
...
function refresh() {
	...
	if(Selection[0].data.D_FACT_FINISH_DATE!=''&&Selection[0].data.D_FACT_FINISH_DATE!=null){
...
function siteChange() {
	...
	if (data.length == 0) {
		...
		for ( var i = 0; i < data.length; i++) {
			...
			Ext.Ajax.request({
				url: AppUrl + 'WorkOrder/pro_run7113_changeordermat',
				method : 'POST',
				params : {
					'A_ID':golid,
					...
					'CHANGEDATE':data.D_DATE_EDITTIME,
					'V_MATERIALCODE':materialcode,
					'a_supplycode':Ext.getCmp('supply').getValue(),
					'a_supplyname':Ext.getCmp('supply').getRawValue(),
					'a_uniquecode':ucode,
					'a_replacedate':Ext.getCmp('changeDate').getSubmitValue() == null ? "": Ext.getCmp('changeDate').getSubmitValue(),
...
function OnClickDeleteLink() {
	...
	Ext.Ajax.request({
		url: AppUrl + 'WorkOrder/pro_run7113_changecancel',
		method : 'POST',
		params : {
			V_I_ID:data.I_ID,
			...
			V_CHANGETIME:data.D_DATE_EDITTIME
		},
		success : function(resp) {
			resp = Ext.decode(resp.responseText);
			if (resp == 'success') {
...
function OnButtonExportClicked() {
	document.location.href=AppUrl + 'excel/GDBJGHGL_EXCEL?V_DEPT_CODE='+ Ext.getCmp("zyq").getValue()+
...
function setsbname() {
	...
	sbxzStore.on('load', function() {
		Ext.getCmp('selEqu').select(data.V_EQUCODE);
	});
	Ext.getCmp('sbqueryWin').hide();
}
function sbquery() {
...
function changeSiteEqusiteSearch() {
	...
	Ext.data.StoreManager.lookup('siteStore').on("load", onChangeSiteEqusiteStoreLoad);
}
