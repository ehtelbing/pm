	...
	if (record.data.leaf == true && record.data.parentid != '-1') {
		Ext.Ajax.request({
			...
			params: {
				...
				V_V_PERSONTYPECODE:record.data.craftcode==null?' ':record.data.craftcode,
				V_V_PERSONTYPENAME:record.data.craftname==null?' ':record.data.craftname,
	...
	Ext.Msg.confirm('警告', '您确定要删除该信息?', function(button) {
		if (button != 'yes') {
			...
			Ext.Ajax.request({
				...
				async: false,
				params: {
					V_I_ID:Ext.data.StoreManager.get('gridStore').data.items[colIndex].data.I_ID
				},
				success : function(response) {
...
function OnClickFiniBackButton() {
	...
	if (ss.data.length > 0) {
		var ModifyRecord = Ext.data.StoreManager.get('gridStore')
			.getModifiedRecords();
