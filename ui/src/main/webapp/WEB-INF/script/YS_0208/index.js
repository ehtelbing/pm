...
Ext.onReady(function () {
    ...
    var gridStore = Ext.create('Ext.data.Store', {
        ...
        pageSize: 20,
        storeId: 'gridStore',
        fields: ['V_ORGCODE','V_ORGNAME', 'V_P_MAT', 'V_A_MAT', 'V_PER', 'V_TOOL', 'V_TOTAL_P_MAT', 'V_TOTAL_A_MAT'],
        proxy: {
            type: 'ajax',
            actionMethods: {
                read: 'POST'
            },
            url: AppUrl + 'YS/YS_CHARGE_WORKORDER_ORG_SEL',
            reader: {
                type: 'json',
                root: 'list',
                total: 'total'
    ...
    var gridStore2 = Ext.create('Ext.data.Store', {
        ...
        proxy: {
            ...
            url: AppUrl + 'YS/YS_CHARGE_WORKORDER_DEPT_SEL',
            reader: {
                type: 'json',
                root: 'list',
                total: 'total'
    ...
    var dialog = Ext.create('Ext.window.Window', {
        id: 'dialog',
        title: '<div align="center">厂矿结算数据汇总表</div>',
        width: 1200,
        height: 600,
        modal: true,
        plain: true,
    ...
    var dialog3 = Ext.create('Ext.window.Window', {
        id: 'dialog3',
        title: '<div align="center">作业区结算数据明细表</div>',
        width: 850,
        height: 600,
        modal: true,
        plain: true,
...
function _querySum(){
    Ext.Ajax.request({
        url: AppUrl + 'YS/YS_CHARGE_W_ORG_TOTAL_SEL',
        async: false,
        method: 'POST',
        params: {
...
function _querySum2(){
    Ext.Ajax.request({
        ...
        success: function (resp) {
            var data = Ext.decode(resp.responseText);
            if (data.RET=="success") {
                Ext.getCmp('SUM_P_MAT2').setValue(data.RET1);
                Ext.getCmp('SUM_A_MAT2').setValue(data.RET2);
                Ext.getCmp('SUM_PER2').setValue(data.RET3);
                Ext.getCmp('SUM_TOOL2').setValue(data.RET4);
                Ext.getCmp('SUM_TOTAL_P_CHARGE2').setValue(data.RET5);
                Ext.getCmp('SUM_TOTAL_A_CHARGE2').setValue(data.RET6);
            }
        }
    });
}
