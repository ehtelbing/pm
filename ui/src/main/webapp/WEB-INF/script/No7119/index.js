...
var gridStore = Ext.create("Ext.data.Store", {
    ...
    proxy : {
        type : 'ajax',
        async : false,
        url : AppUrl + 'lx/PRO_RUN_SITE_BJ_ALL',
        actionMethods : {
            read : 'POST'
        },
        reader : {
...
Ext.onReady(function() {
    ...
    Ext.data.StoreManager.lookup('ckStore').on(
        'load',
        function() {
            /*Ext.getCmp('ck').store.insert(0, {
                'V_DEPTCODE' : '%',
                'V_DEPTNAME' : '全部'
            });*/
            Ext.getCmp('ck').store.sort('V_DEPTCODE', 'ASC');
            Ext.getCmp('ck').select(
                Ext.data.StoreManager.lookup('ckStore').getAt(0));
...
function VGShow() {
    ...
    Ext.Ajax.request({
        url: AppUrl + 'lx/pro_run7119_sitevgurl',
        type: 'ajax',
        method: 'POST',
        params: {
...
//             if(resp!=""){
//                 Ext.window.Window(resp,window, "dialogWidth:800px;dialogHeight:420px");
//             }else{
//                 Ext.example.msg('操作信息', '{0}', '没有找到相应VG图');
//             }
//         }
//
//     });
...
function exportexcel() {
    ...
    var IN_EQUID = Ext.getCmp('xzsb').getValue();
    var IN_PLANT = Ext.getCmp('ck').getValue();
    var IN_DEPART = Ext.getCmp('zyq').getValue();
    var IN_STATUS = Ext.getCmp('bjzt').getValue();
    var IN_BJCODE = Ext.getCmp('bjbm').getValue();
    var IN_BJDESC = Ext.getCmp('bjms').getValue();

    document.location.href = AppUrl + 'lx/PRO_RUN_SITE_BJ_ALL_EXCLE?IN_EQUID='+ encodeURI(encodeURI(IN_EQUID)) +
        '&IN_PLANT='+ encodeURI(encodeURI(IN_PLANT)) +
        '&IN_DEPART=' + encodeURI(encodeURI(IN_DEPART)) +
        '&IN_STATUS=' + encodeURI(encodeURI(IN_STATUS)) +
        '&IN_BJCODE=' + encodeURI(encodeURI(IN_BJCODE)) +
        '&IN_BJDESC=' + encodeURI(encodeURI(IN_BJDESC)) ;
}

function beforeloadStore(store){
    store.proxy.extraParams.parVal = [uniqueid];
}
function loadgridStore(store){
        store.proxy.extraParams.IN_EQUID = Ext.getCmp('xzsb').getValue();
        store.proxy.extraParams.IN_PLANT = Ext.getCmp('ck').getValue();
        store.proxy.extraParams.IN_DEPART = Ext.getCmp('zyq').getValue();
        store.proxy.extraParams.IN_STATUS =  Ext.getCmp('bjzt').getValue();
        store.proxy.extraParams.IN_BJCODE =  Ext.getCmp('bjbm').getValue();
        store.proxy.extraParams.IN_BJDESC = Ext.getCmp('bjms').getValue()
}