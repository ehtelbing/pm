...
var sbxzStore = Ext.create('Ext.data.Store', {
    ...
    proxy : {
        type : 'ajax',
        async : false,
        url : AppUrl + 'Zyk/PRO_RUN7111_EQULIST',
        actionMethods : {
            read : 'POST'
        },
        reader : {
...
var fzrStore = Ext.create('Ext.data.Store', {
    ...
    proxy : {
        type : 'ajax',
        async : false,
        url : AppUrl + 'Zyk/PRO_RUN7115_PERSONLIST',
        actionMethods : {
            read : 'POST'
        },
        reader : {
...
var gridStore = Ext.create("Ext.data.Store", {
    ...
    proxy : {
        type : 'ajax',
        async : false,
        url : AppUrl + 'Zyk/PRO_RUN7115_SELECT',
        actionMethods : {
            read : 'POST'
        },
        reader : {
...
function OnButtonExportClicked() {
    document.location.href = AppUrl + 'Zyk/PRO_RUN7115_SELECT_TABLE?V_V_DEPARTCODE='+ excelCS(Ext.getCmp('zyq').getValue()) +
        '&V_V_PLANTCODE=' + excelCS(Ext.util.Cookies.get('v_orgCode')) +
        '&V_V_BJ_ID=' + excelCS(Ext.getCmp('xzsb').getValue()) +
        '&V_V_USERID=' + excelCS(Ext.getCmp('fzr').getValue()) ;
}
