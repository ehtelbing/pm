...
var bjStore = Ext.create('Ext.data.Store', {
	...
	proxy : {
		type : 'ajax',
		async : false,
		url : AppUrl + 'PM_12/PRO_RUN7117_BJLIST',
		actionMethods : {
			read : 'POST'
		},
		reader : {
...
var grid = Ext.create('Ext.grid.Panel', {
	...
	store : {
		...
		proxy : {
			type : 'ajax',
			async : false,
			url : AppUrl + 'PM_12/PRO_RUN7117_BJWORKLIST',
			actionMethods : {
				read : 'POST'
			},
			reader : {
...
Ext.onReady(function() {
	...
	Ext.data.StoreManager.lookup('bjStore').on('load', function() {
		Ext.getCmp('bjDesc').select(Ext.data.StoreManager.lookup('bjStore').getAt(0));
	});
...
function OnClickDeleteLink() {
	...
	window.showModalDialog(AppUrl + '/' + data.url, null, "dialogWidth=650px;dialogHeight=400px");
}

function OnButtonExportClicked() {
	document.location.href=AppUrl + 'excel/BJYXTJ_EXCEL?V_V_DEPARTCODE='+encodeURI(Ext.getCmp('zyq').getValue())+
	'&V_V_PLANTCODE='+encodeURI(Ext.util.Cookies.get('v_orgCode'))+
	'&V_V_BJ_ID='+encodeURI(Ext.getCmp('bjDesc').getValue());
}
