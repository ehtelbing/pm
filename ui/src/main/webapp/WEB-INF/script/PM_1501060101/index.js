...
if (location.href.split('?')[1] != null) {
    ...
    MEND_CODE = Ext.urlDecode(location.href.split('?')[1]).MEND_CODE;
}
    ...
    Ext.Ajax.request({
        ...
        success: function (response) {
            ...
            if (data.RET.length > 0) {
                ...
                Ext.getCmp('SEND_PLANTNAME').setValue(data.RET[0].APPLY_PLANTNAME);
                Ext.getCmp('MEND_TYPE_IN').setValue(data.RET[0].MEND_TYPE);
...
function _save() {
    if (Ext.getCmp('DJ_UQ_CODE_IN').getSubmitValue() == '') {
        Ext.MessageBox.show({
            title: '提示',
            msg: '电机编号不能为空!',
            buttons: Ext.MessageBox.OK,
    ...
    if (Ext.getCmp('DJ_NAME_IN').getSubmitValue() == '') {
        Ext.MessageBox.show({
            title: '提示',
            msg: '电机名称不能为空!',
            buttons: Ext.MessageBox.OK,
    ...
    Ext.Ajax.request({
        url: AppUrl + 'mwd/PRO_DJ601_SAVEORDER',
        type: 'ajax',
        method: 'POST',
        params: {
            'DJ_UQ_CODE_IN': DJ_UQ_CODE,
            'DJ_NAME_IN': DJ_NAME,
            'APPLY_ID_IN': APPLY_ID,
            ...
            'MENDDEPT_CODE_IN': Ext.getCmp('MENDDEPT_CODE_IN').getValue(),
            'MEND_USERID_IN': Ext.getCmp('MEND_USERID_IN').getSubmitValue(),
            'MEND_USERNAME_IN': Ext.getCmp('MEND_USERID_IN').getRawValue(),
